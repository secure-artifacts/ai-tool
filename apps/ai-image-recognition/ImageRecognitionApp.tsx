...
import { convertBlobToBase64, parsePasteInput, parsePasteInputGrouped, fetchImageBlob, extractUrlsFromHtml, extractUrlsFromHtmlGrouped, isLikelyHotlinkBlocked } from './utils';
...
import {
    ...
    applyOverridesToCombination,
    getDefaultExtractPrompt,
    buildMultiDimensionExtractPrompt,
...
const ImageRecognitionApp: React.FC<ImageRecognitionAppProps> = ({
    ...
    // === 快捷模式：维度覆盖标签 ===
    // 覆盖值：支持手动输入、独立参考图提取、队列逐图提取三种模式
    const [quickOverrides, setQuickOverrides] = useState<Record<string, OverrideEntry>>({}); // 库名 → 覆盖配置
    const [editingOverrideLib, setEditingOverrideLib] = useState<string | null>(null); // 当前正在编辑的库名
    const [editingOverrideSnapshot, setEditingOverrideSnapshot] = useState<string>(''); // 编辑前快照（用于判断是否改动）
    const [expandedOverrideCountLib, setExpandedOverrideCountLib] = useState<string | null>(null); // 覆盖个数编辑展开状态
    const [extractPromptModal, setExtractPromptModal] = useState<{ libName: string; mode: 'image' | 'queue-image' } | null>(null);
    const quickOverridesRef = useRef<Record<string, OverrideEntry>>({});
    useEffect(() => { quickOverridesRef.current = quickOverrides; }, [quickOverrides]);
    const getOverrideEditSignature = useCallback((entry?: OverrideEntry): string => {
        const imageLibrarySig = (entry?.imageLibrary || [])
            .map(img => `${img.id}:${img.mimeType}:${img.data.length}`)
            .join('|');
        return JSON.stringify({
            mode: entry?.mode || 'text',
            value: entry?.value || '',
            count: entry?.count || 0,
            extractPrompt: entry?.extractPrompt || '',
            autoExtract: !!entry?.autoExtract,
            imageDataLen: entry?.imageData?.length || 0,
            imageMimeType: entry?.imageMimeType || '',
            imageLibrarySig
        });
    }, []);
    const closeOverrideEditor = useCallback(() => {
        setEditingOverrideLib(null);
        setEditingOverrideSnapshot('');
        setExpandedOverrideCountLib(null);
    }, []);
    const openOverrideEditor = useCallback((libName: string, entry?: OverrideEntry) => {
        setEditingOverrideLib(libName);
        setEditingOverrideSnapshot(getOverrideEditSignature(entry));
        setExpandedOverrideCountLib(null);
        ...
        setState(prev => {
            ...
            if (currentTabIdx >= 0) {
                updatedTabs[currentTabIdx] = {
                    ...
                    // 标签页独立配置保存
                    workMode: prev.workMode || 'standard',
                    viewMode: prev.viewMode,
                    creativeCount: prev.creativeCount,
                    creativeResults: prev.creativeResults,
                    creativeInstruction: prev.creativeInstruction,
            ...
            return {
                ...
                isProcessing: false,
                // 恢复标签页独立配置
                workMode: targetTab.workMode || 'standard',
                viewMode: targetTab.viewMode || prev.viewMode,
                creativeCount: targetTab.creativeCount ?? 4,
                creativeResults: targetTab.creativeResults || [],
                creativeInstruction: targetTab.creativeInstruction || '',
    ...
    const handleTabAdd = useCallback(() => {
        ...
        setState(prev => {
            ...
            return {
                ...
                viewMode: prev.viewMode, // 视图模式继承当前
                creativeCount: 4,
                creativeResults: [],
                creativeInstruction: '',
    ...
    const handleTabRemove = useCallback((tabId: string) => {
        ...
        setState(prev => {
            ...
            let newState: Partial<typeof prev> = {};

            // 如果删除的是当前活动标签页，切换到相邻的标签页并恢复其配置
            if (tabId === prev.activeTabId) {
                const newActiveTab = newTabs[Math.min(tabIdx, newTabs.length - 1)];
                newState = {
                    activeTabId: newActiveTab.id,
                    images: newActiveTab.images,
                    prompt: newActiveTab.prompt,
                    innovationInstruction: newActiveTab.innovationInstruction,
                    globalInnovationTemplateId: newActiveTab.globalInnovationTemplateId,
                    globalInnovationCount: newActiveTab.globalInnovationCount,
                    globalInnovationRounds: newActiveTab.globalInnovationRounds,
                    // 恢复标签页独立配置
                    workMode: newActiveTab.workMode || 'standard',
                    viewMode: newActiveTab.viewMode || prev.viewMode,
                    creativeCount: newActiveTab.creativeCount ?? 4,
                    creativeResults: newActiveTab.creativeResults || [],
                    creativeInstruction: newActiveTab.creativeInstruction || '',
                ...
                // 恢复随机库配置
                pendingRandomLibraryConfigRef.current = newActiveTab.randomLibraryConfig || DEFAULT_RANDOM_LIBRARY_CONFIG;
            }
    ...
    const isGyazoDirectImageUrl = useCallback((url?: string) => {
        if (!url) return false;
        return /^https?:\/\/i\.gyazo\.com\/[a-f0-9]+\.(png|jpg|gif)/i.test(url);
    }, []);
                ...
                try {
                    ...
                    setState(prev => ({
                        ...
                        images: prev.images.map(i =>
                            i.id === img.id
                                ? { ...i, imageUrl: blobUrl, base64Data: base64, mimeType: mimeType }
                                : i
                        )
                    }));
                } catch (error) {
    ...
    const extractDimensionsFromImage = async (
        ...
        imageSource?: string
    ): Promise<{ result: Record<string, string>; logs: Array<{ timestamp: number; prompt: string; response: string; label?: string; imageSource?: string }> }> => {
        if (dimensions.length === 0) return { result: {}, logs: [] };

        const prompt = buildMultiDimensionExtractPrompt(dimensions);
        const resultText = await classifyImage(base64Data, mimeType, prompt);

        const logs = [{
            timestamp: Date.now(),
            prompt,
            response: resultText,
            label: `维度提取 (${dimensions.map(d => d.libName).join(', ')})`,
            imageSource
        }];

        if (dimensions.length === 1) {
            // 单维度：直接使用返回值
            return { result: { [dimensions[0].libName]: resultText.trim() }, logs };
        }

        // 多维度：解析格式化结果
        const parsed = parseMultiDimensionExtractResult(resultText, dimensions.map(d => d.libName));

        // 如果解析失败（没有匹配到格式），尝试按行分配
        if (Object.keys(parsed).length === 0) {
            const lines = resultText.split('\n').filter(l => l.trim());
            for (let i = 0; i < Math.min(lines.length, dimensions.length); i++) {
                parsed[dimensions[i].libName] = lines[i].trim();
            }
        }
    ...
    // 解析覆盖配置中的 queue-image 模式 以及 per-card 参考图模式
    const resolveQueueImageOverrides = async (
        overrides: Record<string, OverrideEntry>,
        imageBase64: string,
        imageMimeType: string,
        cardRefSelections?: Record<string, string> // 卡片级参考图选择 (维度名 → RefImage.id)
    ): Promise<{ resolved: Record<string, OverrideEntry>; logs: Array<{ timestamp: number; prompt: string; response: string; label?: string; imageSource?: string }> }> => {
        const resolved = { ...overrides };
        const logs: Array<{ timestamp: number; prompt: string; response: string; label?: string; imageSource?: string }> = [];

        // 1. 处理 queue-image 模式：从当前队列图片提取
        const queueImageDims = Object.entries(overrides)
            .filter(([, v]) => v.mode === 'queue-image')
            .map(([libName, entry]) => ({
                libName,
                extractPrompt: entry.extractPrompt || getDefaultExtractPrompt(libName)
            }));

        if (queueImageDims.length > 0) {
            const { result: extracted, logs: extractLogs } = await extractDimensionsFromImage(imageBase64, imageMimeType, queueImageDims, 'main');
            logs.push(...extractLogs);
            for (const [libName, extractedValue] of Object.entries(extracted)) {
                if (resolved[libName]) {
                    resolved[libName] = { ...resolved[libName], value: extractedValue, mode: 'text' };
                }
            }
        }
        ...
        if (cardRefSelections) {
            ...
            for (const { libName, imgData, imgMime, prompt } of perCardDims) {
                try {
                    ...
                    console.error(`[per-card提取] ${libName} 提取失败:`, err);
                }
            }
        }
...
- 每张图片的结果格式与单张处理时完全相同`,
    ...
    const addEmptyImageCard = useCallback(() => {
        const emptyCard: ImageItem = {
            id: uuidv4(),
            sourceType: 'file',
            originalInput: '空卡片',
            imageUrl: '',
    ...
    const runNoImageBatchInnovation = async () => {
        const cardsToProcess = textCardsRef.current.filter(card => card.topic.trim() && (card.status === 'idle' || card.status === 'error'));
...
- 可以直接用于AI图像生成`;
    ...
    // 分组版：同一行的多张图合并为一张卡片（第一个=主图，其余=融合图）
    const addFromUrlGroups = async (groups: import('./utils').PasteGroup[]) => {
        ...
        for (const group of groups) {
            ...
            if (group.length > 1 && workMode === 'quick') {
                // 快捷模式：同一行的图片合并为一张卡片（主图+融合图）
                item.fusionImages = group.slice(1).map(sub => ({
                    id: uuidv4(),
        ...
        for (const item of newItems) {
            const group = groups.find(g => g.length > 0 && g[0].url === item.fetchUrl);
            // 主图
            (async () => {
                try {
            ...
            if (group && group.length > 1) {
                group.slice(1).forEach((sub, subIdx) => {
                    (async () => {
                        try {
                            const { blob } = await fetchImageBlob(sub.url);
                            const blobUrl = URL.createObjectURL(blob);
                            const base64 = await convertBlobToBase64(blob);
                            setImages(prev => prev.map(img => {
                                if (img.id !== item.id) return img;
                                const fusions = [...(img.fusionImages || [])];
                                if (fusions[subIdx]) fusions[subIdx] = { ...fusions[subIdx], imageUrl: blobUrl, base64Data: base64 };
                                return { ...img, fusionImages: fusions };
                            }));
                        } catch (err) { console.error(`[融合图加载失败] ${sub.url}:`, err); }
                    })();
                });
            }
        }
    ...
    useEffect(() => {
        const handleGlobalPaste = (e: ClipboardEvent) => {
            // 输入焦点在可编辑区域时，不拦截粘贴（避免影响弹窗 textarea / input）
            const activeElement = document.activeElement as HTMLElement | null;
            const activeIsHiddenPasteCapture = activeElement?.getAttribute('aria-hidden') === 'true';
            if (activeElement &&
                (activeElement.tagName === 'INPUT' || activeElement.tagName === 'TEXTAREA' || activeElement.isContentEditable) &&
                !activeIsHiddenPasteCapture) {
                return;
            }
            ...
            if (e.clipboardData) {
                ...
                if (plainText && plainText.includes('=IMAGE')) {
                    e.preventDefault();
                    // 检测是否有 tab 分隔（谷歌表格同行多列）
                    const hasTab = plainText.includes('\t');
                    if (hasTab) {
                        const groups = parsePasteInputGrouped(plainText);
                        addFromUrlGroupsRef.current(groups);
                    } else {
                ...
                if (html) {
                    // 检测 HTML 是否有表格行结构（Google Sheets）
                    ...
                    if (hasTableRows) {
                        ...
                        if (groups.length > 0) {
                            e.preventDefault();
                            // 转换为 PasteGroup 格式用 addFromUrlGroups
                            const pasteGroups: import('./utils').PasteGroup[] = groups.map(group =>
                            ...
                            addFromUrlGroupsRef.current(pasteGroups);
                            handled = true;
                            return;
                        }
                    }
                    ...
                    try {
                        ...
                        if (text && (text.includes('http') || text.includes('=IMAGE'))) {
                            if (text.includes('\t') && text.includes('=IMAGE')) {
                                const groups = parsePasteInputGrouped(text);
                                addFromUrlGroupsRef.current(groups);
                            } else {
                                const parsed = parsePasteInput(text);
                                addFromUrlsRef.current(parsed);
    ...
    // 多图合并为一张卡片（第一个=主图，其余=融合图）
    const handleFilesAsOneCard = async (files: File[]) => {
        const imageFiles = files.filter(f => f.type.startsWith('image/'));
        if (imageFiles.length === 0) return;

        const mainFile = imageFiles[0];
        const mainBlobUrl = URL.createObjectURL(mainFile);
        const mainBase64 = await convertBlobToBase64(mainFile);
        const id = uuidv4();

        const fusionImages: import('./types').FusionSubImage[] = [];
        ...
        const item: ImageItem = {
            ...
            originalInput: mainFile.name,
            imageUrl: mainBlobUrl,
            base64Data: mainBase64,
            mimeType: mainFile.type,
            status: 'idle' as const,
            isUploadingToGyazo: autoUploadGyazo,
            chatHistory: [],
            fusionImages: fusionImages.length > 0 ? fusionImages : undefined
        };
    ...
    const handleFiles = async (files: File[]) => {
        const imageFiles = files.filter(f => f.type.startsWith('image/'));
        // 多图拖拽时弹窗选择（仅在 creative/quick 模式下）
        if (imageFiles.length >= 2 && (workMode === 'creative' || workMode === 'quick')) {
            setPendingDropFiles(imageFiles);
            return;
        }
    ...
    const addFromUrlGroupsRef = useRef(addFromUrlGroups);
    const handleFilesRef = useRef(handleFiles);

    useEffect(() => {
        addFromUrlsRef.current = addFromUrls;
        addFromUrlGroupsRef.current = addFromUrlGroups;
    ...
    const toggleDescribeFirst = useCallback((id: string, value: boolean) => {
        setImages(prev => prev.map(img =>
            img.id === id ? { ...img, needDescribeFirst: value } : img
        ));
    }, [setImages]);
    ...
    const updateCardRefSelection = useCallback((cardId: string, dimName: string, refImageId: string | null) => {
        setImages(prev => prev.map(img => {
            ...
            const selections = { ...(img.overrideRefSelections || {}) };
            if (refImageId) {
                selections[dimName] = refImageId;
            } else {
                delete selections[dimName];
            }
            return { ...img, overrideRefSelections: selections };
        }));
    }, [setImages]);
    ...
    const updateCardOverrideCount = useCallback((cardId: string, dimName: string, count: number | null) => {
        setImages(prev => prev.map(img => {
            if (img.id !== cardId) return img;
            const counts = { ...(img.overrideCountOverrides || {}) };
            if (count !== null && count >= 0) {
                counts[dimName] = count;
            } else {
                delete counts[dimName];
            }
            return { ...img, overrideCountOverrides: counts };
        }));
    }, [setImages]);

    // 更新卡片级文字覆盖（维度名 → 覆盖文本，null 表示清空）
    const updateCardTextOverride = useCallback((cardId: string, dimName: string, value: string | null) => {
        setImages(prev => prev.map(img => {
            if (img.id !== cardId) return img;
            const next = { ...(img.overrideTextOverrides || {}) };
            const trimmed = value?.trim() || '';
            if (trimmed) {
                next[dimName] = trimmed;
            } else {
                delete next[dimName];
            }
            return {
                ...img,
                overrideTextOverrides: Object.keys(next).length > 0 ? next : undefined
            };
        }));
    }, [setImages]);
    ...
    // 应用卡片级手动文字覆盖（优先级最高，覆盖全局和图片提取结果）
    const applyCardTextOverrides = useCallback((combinations: string[], item: ImageItem): string[] => {
        const textOverrides = item.overrideTextOverrides;
        if (!textOverrides || Object.keys(textOverrides).length === 0) return combinations;

        const tempOv: Record<string, OverrideEntry> = {};
        for (const [dimName, value] of Object.entries(textOverrides)) {
            const trimmed = value?.trim();
            if (!trimmed) continue;
            const cardCount = item.overrideCountOverrides?.[dimName];
            const globalCount = quickOverridesRef.current[dimName]?.count ?? 0;
            tempOv[dimName] = {
                value: trimmed,
                count: cardCount !== undefined ? cardCount : globalCount,
                mode: 'text'
            };
        }
        if (Object.keys(tempOv).length === 0) return combinations;
        return applyPartialOverrides(combinations, tempOv);
    }, []);
...
你只需要给我最终的AI描述词就行，不需要其他任何多余的内容。并且英文回复我。`;
    ...
    const runCreativeAnalysis = useCallback(async (targetImageIds?: string[]) => {
        ...
        const autoExtractDims = Object.entries(ov)
            .filter(([, v]) => v.mode === 'image' && v.autoExtract && v.imageData && v.imageMimeType && !v.value?.trim());
        if (autoExtractDims.length > 0) {
            console.log('[自动提取] 开始提取', autoExtractDims.length, '个参考图维度');
            // 合并所有待提取维度，按图片分组（每张参考图可能不同）
            for (const [libName, entry] of autoExtractDims) {
                try {
                    const prompt = entry.extractPrompt || getDefaultExtractPrompt(libName);
                    const result = await classifyImage(entry.imageData!, entry.imageMimeType!, prompt);
                    setQuickOverrides(prev => ({
                        ...prev,
                        [libName]: { ...prev[libName], value: result.trim(), extractedValue: result.trim() }
                    }));
                    // 注意：autoExtract 日志会在后续 per-card conversationLog 中通过 resolveQueueImageOverrides 捕获
                    console.log(`[自动提取] ${libName}: ${result.trim().slice(0, 30)}...`);
                } catch (err) {
                    console.error(`[自动提取] ${libName} 提取失败:`, err);
                }
            }
            // 等待状态更新
                ...
                if (isAdvancedMode) {
                    ...
                    if (workModeRef.current === 'quick') {
                        ...
                        if (hasQueueImage && readyImages[0]?.base64Data && readyImages[0]?.mimeType) {
                            const { resolved: resolvedOv } = await resolveQueueImageOverrides(ov, readyImages[0].base64Data, readyImages[0].mimeType);
                            ov = resolvedOv;
                        }
...
只输出JSON数组，不要其他内容。`;
        ...
        await processWithConcurrency(readyImages, imageConcurrency, async (item: ImageItem) => {
            ...
            try {
                ...
                const imageAppendCfgs = (item.refImageConfigs || [])
                    .filter(cfg => cfg.dimName === QUICK_IMAGE_APPEND_DIM && (cfg.extractPrompt || '').trim())
                    .sort((a, b) => a.imageIndex - b.imageIndex);
                if (imageAppendCfgs.length > 0) {
                    const extractedAppendParts: string[] = [];
                    for (const cfg of imageAppendCfgs) {
                        ...
                        try {
                            const appendResult = await classifyImage(imgBase64, imgMime, cfg.extractPrompt!.trim());
                            const cleaned = appendResult.trim();
                            conversationLog.push({
                                timestamp: Date.now(),
                                prompt: cfg.extractPrompt!.trim(),
                                response: appendResult,
                                label: `图片追加提取 (图${cfg.imageIndex + 1})`,
                                imageSource
                            });
                            if (cleaned) extractedAppendParts.push(cleaned);
                        } catch (err) {
                            console.error(`[图级追加提取] 图${cfg.imageIndex + 1} 提取失败:`, err);
                        }
                    }
                    if (extractedAppendParts.length > 0) {
                        const appendText = extractedAppendParts.join('\n');
                        itemUserInput = itemUserInput ? `${itemUserInput}\n${appendText}` : appendText;
                    }
                }
                ...
                if (isNoImageCard) {
                    ...
                    if (isAdvancedMode) {
                        ...
                        if (workModeRef.current === 'quick') {
                            ...
                            if ((hasQueueImage || hasPerCardRef) && item.base64Data && item.mimeType) {
                                const { resolved: resolvedOv, logs: ovLogs } = await resolveQueueImageOverrides(ov, item.base64Data, item.mimeType, item.overrideRefSelections);
                                ov = resolvedOv;
                        ...
                        if (item.refImageConfigs && item.refImageConfigs.length > 0) {
                            ...
                            for (const [imgIdxStr, cfgs] of Object.entries(imgGrouped)) {
                                ...
                                try {
                                    const { result: extracted, logs: refLogs } = await extractDimensionsFromImage(imgBase64, imgMime, dimsToExtract, imgIdx === 0 ? 'main' : `fusion:${imgIdx - 1}`);
                                    conversationLog.push(...refLogs);
                                    for (const [libName, value] of Object.entries(extracted)) {
                                        if (value?.trim()) overrideMap[libName] = value.trim();
                                    }
                                } catch (err) {
                                    console.error(`[卡片级提取] 图${imgIdx + 1}提取失败:`, err);
                                }

                                // 收集覆盖个数
                                for (const cfg of cfgs) {
                                    if (cfg.overrideCount !== undefined) {
                                        countOverrides[cfg.dimName] = cfg.overrideCount;
                                    }
                                }
                            }

                            if (Object.keys(overrideMap).length > 0) {
                                // 构建一个临时 OverrideEntry 来应用
                                const tempOv: Record<string, import('./services/randomLibraryService').OverrideEntry> = {};
                                for (const [dimName, value] of Object.entries(overrideMap)) {
                                    tempOv[dimName] = {
                                        value,
                                        count: countOverrides[dimName] ?? 0,
                                        mode: 'text'
                                    };
                                }
                                combinations = applyPartialOverrides(combinations, tempOv);
                            }
                        }
                        combinations = applyCardTextOverrides(combinations, item);
                        ...
                        conversationLog.push({
                            timestamp: Date.now(),
                            prompt: `随机库生成了 ${combinations.length} 组关键词组合：\n\n${combinations.map((c, i) => `【组合${i + 1}】${c}`).join('\n')}`,
                            response: `共 ${combinations.length} 组，模式: ${randomLibraryConfigRef.current.combinationMode || 'random'}`,
                            label: '随机库组合'
                        });

                        if (batchAdvancedMode && combinations.length > 1) {
                            // 合并模式：所有随机组合打包为一次API调用
                            const transitionInstruction = DEFAULT_TRANSITION_INSTRUCTION;
                            const priorityInstruction = getPriorityInstruction(!!itemUserInput, true);

                            const combinedInstructions = combinations.map((combo, idx) =>
                                `【变体${idx + 1}】\n${transitionInstruction}\n${combo}`
                            ).join('\n\n');

                            const batchPrompt = `${itemEffectiveInstruction}

${combinedInstructions}

${priorityInstruction}

请根据这张图片，分别按以上${combinations.length}组关键词各生成1个创意变体描述。
返回格式为JSON数组:
[{"en": "完整的英文创意描述1", "zh": "完整的中文翻译1"}, ...]

严格返回${combinations.length}个对象的JSON数组，不要其他内容。`;
                            ...
                            conversationLog.push({ timestamp: Date.now(), prompt: batchPrompt, response: innovationResult, label: `创新(有描述)合并${combinations.length}组`, imageSource: 'main' });
...
只输出JSON对象，不要其他内容。`;
                                ...
                                conversationLog.push({ timestamp: Date.now(), prompt: singlePrompt, response: singleResult, label: `创新(有描述)第${results.length + 1}轮`, imageSource: 'main' });
                                results.push(singleResult);
                            }
...
只输出JSON数组，不要其他内容。`;
                        ...
                        conversationLog.push({ timestamp: Date.now(), prompt: innovationPrompt, response: innovationResult, label: '创新(有描述)', imageSource: 'main' });
                    }
                    ...
                    if (isAdvancedMode) {
                        ...
                        // 卡片级参考图提取：从卡片配置的图提取指定维度（与 needOriginalDesc=true 分支保持一致）
                        if (item.refImageConfigs && item.refImageConfigs.length > 0) {
                            const imgGrouped: Record<number, typeof item.refImageConfigs> = {};
                            for (const cfg of item.refImageConfigs) {
                                if (!cfg.dimName || cfg.dimName === QUICK_IMAGE_APPEND_DIM || cfg.dimName === QUICK_IMAGE_DESCRIBE_DIM) continue;
...
严格返回${combinations.length}个对象的JSON数组，不要其他内容。`;
                            ...
                            conversationLog.push({ timestamp: Date.now(), prompt: batchPrompt, response: innovationResult, label: `创新合并${combinations.length}组`, imageSource: 'main' });
...
只输出JSON对象，不要其他内容。`;
                                ...
                                conversationLog.push({ timestamp: Date.now(), prompt: singlePrompt, response: singleResult, label: `创新第${results.length + 1}轮`, imageSource: 'main' });
                                results.push(singleResult);
                            }
...
只输出JSON数组，不要其他内容。`;
                        ...
                        conversationLog.push({ timestamp: Date.now(), prompt: innovationPrompt, response: innovationResult, label: '创新', imageSource: 'main' });
                    }
                }
                ...
                setImages(prev => prev.map(img =>
                    img.id === item.id ? {
                        ...img,
                        status: 'success',
                        aiConversationLog: [...(img.aiConversationLog || []), ...conversationLog]
                    } : img
                ));
    ...
    const startSingleCardInnovation = useCallback((id: string) => {
        ...
        startInnovation(id);
    }, [images, workMode, runCreativeAnalysis, startInnovation, showToast]);
    ...
    const handleAddFusionImage = useCallback(async (imageId: string, file: File) => {
        ...
        setImages(prev => prev.map(img => {
            ...
            const isEmptyCard = !img.imageUrl && !img.base64Data;
            if (isEmptyCard) {
                return {
                    ...img,
                    sourceType: 'file',
                    originalInput: file.name || img.originalInput || '空卡片',
                    imageUrl,
            ...
            return { ...img, fusionImages: [...(img.fusionImages || []), newFusionImage] };
        }));
    }, []);
    ...
    return (
        <>
                    ...
                    {isToolbarCompact ? (
                        ...
                        <div className="max-w-none mx-auto px-4 py-2 space-y-3">
                            ...
                            <div className={`grid grid-cols-1 lg:grid-cols-5 gap-3 items-start ${workMode === 'quick' && quickViewMode === 'standalone' ? 'hidden' : ''}`}>
                                        ...
                                        {workMode === 'quick' ? (
                                            <>
                                                ...
                                                <div className="flex items-center gap-1.5 flex-wrap">
                                                    ...
                                                    {/* 添加空卡片 */}
                                                    <button
                                                        onClick={addEmptyImageCard}
                                                        disabled={isProcessing}
                                                        className={`px-3 py-1.5 rounded-lg font-bold text-xs flex items-center gap-1.5 transition-all border
                                                            ${isProcessing
                                                                ? 'bg-zinc-800/60 text-zinc-500 border-zinc-700/60 cursor-not-allowed'
                                                                : 'bg-zinc-800 text-cyan-300 hover:bg-zinc-700 border-zinc-700 hover:border-cyan-600/50'
                                                            }`}
                                                        data-tip="新增空卡片"
                                                    >
                                                        <Plus size={13} />
                                                        ...
                                                        <div className="mt-1.5">
                                                            <div className="flex items-center gap-1 overflow-x-auto overflow-y-hidden custom-scrollbar">
                                                                ...
                                                                {enabledLibs.map(lib => {
                                                                    ...
                                                                    const hasOverride = !!(override?.value?.trim()) || mode === 'queue-image';
                                                                    const isEditing = editingOverrideLib === lib.name;
                                                                    const overrideCount = override?.count || 0;
                                                                    const modeIcon = mode === 'image' ? '📷' : mode === 'queue-image' ? '🔄' : '';
                                                                    const isExtracting = override?.isExtracting;
                                                                    const isOverrideDirty = isEditing && getOverrideEditSignature(override) !== editingOverrideSnapshot;
                                                                    const showOverrideCountEditor = isEditing && (
                                                                        expandedOverrideCountLib === lib.name ||
                                                                        (overrideCount > 0) ||
                                                                        isOverrideDirty
                                                                    );
                                                                    ...
                                                                    return (
                                                                        <div key={lib.id} className="flex flex-col">
                                                                            <div className="flex items-center gap-0">
                                                                                <button
                                                                                    onClick={() => {
                                                                                        if (isEditing) {
                                                                                            ...
                                                                                            openOverrideEditor(lib.name, override);
                                                                                        }
                                                                                    }}
                                                                                    className={`text-[10px] px-1.5 py-0.5 border transition-all ${hasOverride
                                                                                        ? mode === 'queue-image'
                                                                                            ? 'bg-blue-900/40 text-blue-300 border-blue-700/60 font-medium rounded-l'
                                                                                            : 'bg-amber-900/40 text-amber-300 border-amber-700/60 font-medium rounded-l'
                                                                                        : 'bg-zinc-800/60 text-zinc-400 border-zinc-700/50 hover:bg-zinc-700/60 hover:text-zinc-300 rounded-l'
                                                                                        }`}
                                                                                    data-tip={
                                                                                        mode === 'queue-image' ? `${lib.name}: 逐图提取模式` :
                                                                                ...
                                                                                >
                                                                                    {mode === 'image' && override?.imageData ? (
                                                                                        <span className="relative mr-0.5 inline-block align-middle group/thumb">
                                                                                            <img src={`data:${override.imageMimeType || 'image/jpeg'};base64,${override.imageData}`} alt="" className="w-3.5 h-3.5 rounded-sm object-cover" />
                                                                                            <img src={`data:${override.imageMimeType || 'image/jpeg'};base64,${override.imageData}`} alt="" className="hidden group-hover/thumb:block absolute bottom-full left-0 mb-1 w-28 h-28 rounded-lg object-cover border-2 border-amber-500/60 shadow-xl z-50 pointer-events-none" />
                                                                                        </span>
                                                                                    ) : modeIcon ? (
                                                                                    ...
                                                                                    {hasOverride && mode !== 'queue-image' && override?.value?.trim() && (
                                                                                        <span className="ml-1 text-amber-200/80 text-[9px] max-w-[80px] truncate inline-block align-bottom" title={override.value}>
                                                                                            {override.value.length > 8 ? override.value.slice(0, 8) + '…' : override.value}
                                                                                        </span>
                                                                                    )}
                                                                                    {hasOverride && overrideCount > 0 && <span className="ml-0.5 text-amber-500/70 text-[9px]">×{overrideCount}</span>}
                                                                                    {isExtracting && <span className="ml-0.5 animate-spin">⏳</span>}
                                                                                </button>
                                                                                {!isEditing && hasOverride ? (
                                                                                    <button
                                                                                        ...
                                                                                        className={`text-[10px] px-1 py-0.5 rounded-r border border-l-0 transition-all ${mode === 'queue-image'
                                                                                            ? 'border-blue-700/60 bg-blue-900/20 text-blue-500 hover:text-red-400 hover:bg-red-900/20'
                                                                                            : 'border-amber-700/60 bg-amber-900/20 text-amber-500 hover:text-red-400 hover:bg-red-900/20'
                                                                                            }`}
                                                                                        data-tip="清除覆盖"
                                                                                    >×</button>
                                                                                ) : !isEditing ? (
                                                                            ...
                                                                            {/* 展开的编辑面板 */}
                                                                            {isEditing && (
                                                                                <div className="group/edit-panel mt-1 p-2 bg-zinc-900/90 border border-zinc-700/60 rounded-lg text-[10px] min-w-[220px] shadow-xl">
                                                                                    {/* 模式切换 */}
                                                                                    <div className="flex gap-1 mb-1.5">
                                                                                        {(['text', 'image', 'queue-image'] as const).map(m => (
                                                                                            <button
                                                                                                key={m}
                                                                                                onClick={() => setQuickOverrides(prev => ({
                                                                                                    ...prev,
                                                                                                    [lib.name]: { ...prev[lib.name], value: prev[lib.name]?.value || '', count: prev[lib.name]?.count || 0, mode: m, extractPrompt: prev[lib.name]?.extractPrompt || getDefaultExtractPrompt(lib.name) }
                                                                                                }))}
                                                                                                className={`px-1.5 py-0.5 rounded text-[9px] border transition-all ${mode === m
                                                                                                    ? 'bg-amber-800/50 text-amber-200 border-amber-600/60'
                                                                                                    : 'bg-zinc-800/60 text-zinc-500 border-zinc-700/40 hover:text-zinc-300'
                                                                                                    }`}
                                                                                            >
                                                                                                {m === 'text' ? '✏️手动' : m === 'image' ? '📷参考图' : '🔄逐图提取'}
                                                                                            </button>
                                                                                        ))}
                                                                                    </div>
                                                                                    ...
                                                                                    {mode === 'text' && (
                                                                                        <div className="flex items-center gap-1">
                                                                                            <input
                                                                                                ...
                                                                                                onChange={(e) => setQuickOverrides(prev => ({ ...prev, [lib.name]: { ...prev[lib.name], value: e.target.value, count: prev[lib.name]?.count || 0 } }))}
                                                                                                onKeyDown={(e) => { if (e.key === 'Enter') closeOverrideEditor(); }}
                                                                                                placeholder={`输入${lib.name}固定值...`}
                                                                                                className="flex-1 px-1.5 py-0.5 text-[10px] bg-zinc-800 border border-zinc-600/50 rounded text-amber-200 placeholder-zinc-600 focus:outline-none focus:border-amber-500/60"
                                                                                            />
                                                                                    ...
                                                                                    {mode === 'image' && (() => {
                                                                                        ...
                                                                                        const addImageToLibrary = (base64: string, mimeType: string) => {
                                                                                            const newImg: RefImage = { id: `ref-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`, data: base64, mimeType };
                                                                                            setQuickOverrides(prev => {
                                                                                                const entry = prev[lib.name] || { value: '', count: 0, mode: 'image' as const };
                                                                                                const newLibrary = [...(entry.imageLibrary || []), newImg];
                                                                                                // 同时保持 imageData 兼容（用第一张）
                                                                                                return { ...prev, [lib.name]: { ...entry, mode: 'image' as const, imageLibrary: newLibrary, imageData: newLibrary[0]?.data, imageMimeType: newLibrary[0]?.mimeType, extractPrompt: entry.extractPrompt || getDefaultExtractPrompt(lib.name) } };
                                                                                            });
                                                                                        };
                                                                                        const removeFromLibrary = (refId: string) => {
                                                                                            setQuickOverrides(prev => {
                                                                                                const entry = prev[lib.name];
                                                                                                if (!entry) return prev;
                                                                                                const newLibrary = (entry.imageLibrary || []).filter(img => img.id !== refId);
                                                                                                return { ...prev, [lib.name]: { ...entry, imageLibrary: newLibrary, imageData: newLibrary[0]?.data || '', imageMimeType: newLibrary[0]?.mimeType || '' } };
                                                                                            });
                                                                                            // 同时清理卡片中引用了该图的选择
                                                                                            setImages(prev => prev.map(img => {
                                                                                                if (img.overrideRefSelections?.[lib.name] === refId) {
                                                                                                    const newSel = { ...img.overrideRefSelections };
                                                                                                    delete newSel[lib.name];
                                                                                                    return { ...img, overrideRefSelections: newSel };
                                                                                                }
                                                                                                return img;
                                                                                            }));
                                                                                        };
                                                                                        return (
                                                                                            <div className="space-y-1.5">
                                                                                                <div className="text-zinc-500">提取要求：</div>
                                                                                                <input
                                                                                                    type="text"
                                                                                                    value={override?.extractPrompt || getDefaultExtractPrompt(lib.name)}
                                                                                                    onChange={(e) => setQuickOverrides(prev => ({ ...prev, [lib.name]: { ...prev[lib.name], value: prev[lib.name]?.value || '', count: prev[lib.name]?.count || 0, mode: 'image', extractPrompt: e.target.value } }))}
                                                                                                    onDoubleClick={() => setExtractPromptModal({ libName: lib.name, mode: 'image' })}
                                                                                                    className="w-full px-1.5 py-0.5 text-[10px] bg-zinc-800 border border-zinc-600/50 rounded text-zinc-300 placeholder-zinc-600 focus:outline-none focus:border-amber-500/60"
                                                                                                    title="双击弹框编辑"
                                                                                                />
                                                                                                {/* 参考图库缩略图 */}
                                                                                                {library.length > 0 && (
                                                                                                    <div className="flex flex-wrap gap-1">
                                                                                                        {library.map((img, idx) => (
                                                                                                            <div key={img.id} className="relative group/refimg">
                                                                                                                <img src={`data:${img.mimeType};base64,${img.data}`} alt={`参考${idx + 1}`} className="w-10 h-10 rounded object-cover border border-zinc-600/50" />
                                                                                                                <span className="absolute bottom-0 left-0 bg-black/70 text-[8px] text-zinc-300 px-0.5 rounded-br">{idx + 1}</span>
                                                                                                                <button
                                                                                                                    onClick={() => removeFromLibrary(img.id)}
                                                                                                                    className="absolute -top-1 -right-1 w-3.5 h-3.5 rounded-full bg-red-600 text-white text-[8px] leading-none hidden group-hover/refimg:flex items-center justify-center"
                                                                                                                >×</button>
                                                                                                            </div>
                                                                                                        ))}
                                                                                                    </div>
                                                                                                ...
                                                                                                {/* 添加参考图 */}
                                                                                                <div
                                                                                                    className="border border-dashed rounded p-1.5 text-center cursor-pointer transition-all border-zinc-600/50 hover:border-amber-600/50 hover:bg-zinc-800/50"
                                                                                                    onClick={() => {
                                                                                                        const input = document.createElement('input');
                                                                                                        input.type = 'file';
                                                                                                        input.accept = 'image/*';
                                                                                                        ...
                                                                                                        input.onchange = (e) => {
                                                                                                            ...
                                                                                                            Array.from(files).forEach(file => {
                                                                                                                ...
                                                                                                                reader.onload = () => {
                                                                                                                    ...
                                                                                                                    addImageToLibrary(base64, file.type);
                                                                                                                };
                                                                                                                reader.readAsDataURL(file);
                                                                                                            });
                                                                                                        };
                                                                                                    ...
                                                                                                    tabIndex={0}
                                                                                                >
                                                                                                    <span className="text-zinc-500 text-[10px]">{library.length > 0 ? '➕ 继续添加参考图' : '点击上传 / Ctrl+V 粘贴参考图'}</span>
                                                                                                </div>
                                                                                                {library.length > 1 && (
                                                                                                    <div className="text-[9px] text-blue-400/80">
                                                                                                        📌 已上传{library.length}张参考图，每张卡片可在缩略图旁选择用哪张
                                                                                                    </div>
                                                                                                )}
                                                                                                {/* 提取按钮 + 自动提取开关（仅单张时可直接提取） */}
                                                                                                {library.length === 1 && (
                                                                                                    <div className="flex items-center gap-1">
                                                                                                        <button
                                                                                                            onClick={async () => {
                                                                                                                const img = library[0];
                                                                                                                setQuickOverrides(prev => ({ ...prev, [lib.name]: { ...prev[lib.name], isExtracting: true } as any }));
                                                                                                                try {
                                                                                                                    const prompt = override?.extractPrompt || getDefaultExtractPrompt(lib.name);
                                                                                                                    const result = await classifyImage(img.data, img.mimeType, prompt);
                                                                                                                    setQuickOverrides(prev => ({
                                                                                                                        ...prev,
                                                                                                                        [lib.name]: { ...prev[lib.name], value: result.trim(), extractedValue: result.trim(), isExtracting: false } as any
                                                                                                                    }));
                                                                                                                } catch (err) {
                                                                                                                    console.error('提取失败:', err);
                                                                                                                    setQuickOverrides(prev => ({ ...prev, [lib.name]: { ...prev[lib.name], isExtracting: false } as any }));
                                                                                                                }
                                                                                                            }}
                                                                                                            disabled={isExtracting}
                                                                                                            className="flex-1 px-2 py-1 rounded bg-amber-700/40 text-amber-200 border border-amber-600/40 hover:bg-amber-600/40 disabled:opacity-50 transition-all"
                                                                                                        >
                                                                                                            {isExtracting ? '⏳ 提取中...' : '🔍 立即提取'}
                                                                                                        </button>
                                                                                                        <button
                                                                                                            onClick={() => setQuickOverrides(prev => ({
                                                                                                                ...prev,
                                                                                                                [lib.name]: { ...prev[lib.name], autoExtract: !prev[lib.name]?.autoExtract } as any
                                                                                                            }))}
                                                                                                            className={`px-1.5 py-1 rounded border text-[9px] transition-all whitespace-nowrap ${override?.autoExtract
                                                                                                                ? 'bg-green-800/40 text-green-300 border-green-600/50'
                                                                                                                : 'bg-zinc-800/60 text-zinc-500 border-zinc-700/40 hover:text-zinc-300'
                                                                                                                }`}
                                                                                                            data-tip="开启后点击「开始」时自动从参考图提取"
                                                                                                        >
                                                                                                            {override?.autoExtract ? '⚡自动' : '手动'}
                                                                                                        </button>
                                                                                                    </div>
                                                                                                )}
                                                                                                {library.length > 1 && (
                                                                                                    <div className="flex items-center gap-1">
                                                                                                        <button
                                                                                                            onClick={() => setQuickOverrides(prev => ({
                                                                                                                ...prev,
                                                                                                                [lib.name]: { ...prev[lib.name], autoExtract: true } as any
                                                                                                            }))}
                                                                                                            className="flex-1 px-2 py-1 rounded bg-green-800/40 text-green-300 border border-green-600/40 text-[10px]"
                                                                                                        >
                                                                                                            ⚡ 开始时自动按卡片选择提取
                                                                                                        </button>
                                                                                                    </div>
                                                                                    ...
                                                                                    {/* 提取结果（可编辑） */}
                                                                                    {override?.value && mode === 'image' && (
                                                                                        <div className="flex items-center gap-1">
                                                                                            <span className="text-zinc-500 shrink-0">结果:</span>
                                                                                            <input
                                                                                                type="text"
                                                                                                value={override.value}
                                                                                                onChange={(e) => setQuickOverrides(prev => ({ ...prev, [lib.name]: { ...prev[lib.name], value: e.target.value } as any }))}
                                                                                                className="flex-1 px-1.5 py-0.5 text-[10px] bg-zinc-800 border border-green-600/40 rounded text-green-300 focus:outline-none"
                                                                                            />
                                                                                        </div>
                                                                                    )}
                                                                                    {/* 批量应用到其他维度 */}
                                                                            ...
                                                                            {/* 队列逐图提取模式 */}
                                                                            {isEditing && mode === 'queue-image' && (
                                                                                <div className="space-y-1.5">
                                                                                    <div className="text-blue-400">🔄 批量时从每张队列图片自动提取此维度</div>
                                                                                    <div className="text-zinc-500">提取要求：</div>
                                                                                    <input
                                                                                        type="text"
                                                                                        value={override?.extractPrompt || getDefaultExtractPrompt(lib.name)}
                                                                                        onChange={(e) => setQuickOverrides(prev => ({ ...prev, [lib.name]: { ...prev[lib.name], value: '', count: prev[lib.name]?.count || 0, mode: 'queue-image', extractPrompt: e.target.value } }))}
                                                                                        onDoubleClick={() => setExtractPromptModal({ libName: lib.name, mode: 'queue-image' })}
                                                                                        className="w-full px-1.5 py-0.5 text-[10px] bg-zinc-800 border border-blue-600/40 rounded text-zinc-300 placeholder-zinc-600 focus:outline-none focus:border-blue-500/60"
                                                                                        title="双击弹框编辑"
                                                                                    />
                                                                                    <div className="text-zinc-600 text-[9px]">处理每张图时，AI 会先提取该维度描述，再替换到随机组合中</div>
                                                                                </div>
                                                                            )}

                                                                            {isEditing && (showOverrideCountEditor ? (
                                                                                <div className="mt-1.5 pt-1.5 border-t border-zinc-800/70 flex items-center justify-between gap-2">
                                                                                    <span className="text-zinc-500 shrink-0">覆盖个数:</span>
                                                                                    <div className="flex items-center gap-1">
                                                                                        <button
                                                                                            onClick={() => setQuickOverrides(prev => ({
                                                                                                ...prev,
                                                                                                [lib.name]: { ...prev[lib.name], value: prev[lib.name]?.value || '', count: Math.max(0, (prev[lib.name]?.count || 0) - 1) }
                                                                                            }))}
                                                                                        ...
                                                                                        >-</button>
                                                                                        <span className="text-[10px] text-amber-300 min-w-[16px] text-center">{(override?.count || 0) === 0 ? '全' : (override?.count || 0)}</span>
                                                                                        <button
                                                                                            onClick={() => setQuickOverrides(prev => ({
                                                                                                ...prev,
                                                                                                [lib.name]: { ...prev[lib.name], value: prev[lib.name]?.value || '', count: (prev[lib.name]?.count || 0) + 1 }
                                                                                            }))}
                                                                                            className="w-4 h-4 rounded bg-zinc-800 border border-zinc-600/50 text-amber-400 hover:bg-zinc-700 text-[9px] flex items-center justify-center"
                                                                                        >+</button>
                                                                                    </div>
                                                                                ...
                                                                                <div className="mt-1.5 flex justify-end">
                                                                                    <button
                                                                                        onClick={() => setExpandedOverrideCountLib(lib.name)}
                                                                                        className="px-2 py-0.5 rounded text-[10px] bg-zinc-800 text-zinc-300 border border-zinc-700/50 hover:bg-zinc-700 transition-all"
                                                                                    >
                                                                                        覆盖个数
                                                                                    </button>
                                                                                </div>
                                                                ...
                                                                {activeCount > 0 && (
                                                                    <button
                                                                        ...
                                                                        className="text-[10px] px-1.5 py-0.5 rounded bg-red-900/20 text-red-400 border border-red-800/40 hover:bg-red-800/30 transition-all ml-1"
                                                                        data-tip="清除所有覆盖"
                                                                    >
                                                                        清除{activeCount}项
                                                                    </button>
                                                            ...
                                                            {/* 多张参考图时：卡片级参考图分配 */}
                                                            {(() => {
                                                                const dimsWithLibrary = enabledLibs.filter(lib => {
                                                                    const ov = quickOverrides[lib.name];
                                                                    return ov?.mode === 'image' && (ov?.imageLibrary?.length || 0) > 1;
                                                                });
                                                                if (dimsWithLibrary.length === 0 || images.length === 0) return null;
                                                                return (
                                                                    <div className="mt-1.5 p-1.5 bg-zinc-800/40 rounded border border-blue-800/30">
                                                                        <div className="text-[9px] text-blue-400 mb-1">📌 每张卡片的参考图选择：</div>
                                                                        <div className="max-h-32 overflow-y-auto space-y-0.5">
                                                                            {images.map((img, imgIdx) => (
                                                                                <div key={img.id} className="flex items-center gap-1 text-[9px]">
                                                                                    <span className="text-zinc-500 w-4 shrink-0">{imgIdx + 1}.</span>
                                                                                    {img.imageUrl && (
                                                                                        <img src={img.imageUrl} alt="" className="w-4 h-4 rounded object-cover shrink-0" />
                                                                                    )}
                                                                                    {dimsWithLibrary.map(lib => {
                                                                                        const ov = quickOverrides[lib.name]!;
                                                                                        const library = ov.imageLibrary!;
                                                                                        const selectedRefId = img.overrideRefSelections?.[lib.name] || '';
                                                                                        return (
                                                                                            <select
                                                                                                key={lib.name}
                                                                                                value={selectedRefId}
                                                                                                onChange={(e) => updateCardRefSelection(img.id, lib.name, e.target.value || null)}
                                                                                                className="flex-1 min-w-0 px-1 py-0.5 bg-zinc-900 border border-zinc-700/50 rounded text-zinc-300 text-[9px] focus:outline-none focus:border-blue-500/50"
                                                                                                title={`${lib.name} 参考图`}
                                                                                            >
                                                                                                <option value="">{lib.name}: 默认</option>
                                                                                                {library.map((refImg, refIdx) => (
                                                                                                    <option key={refImg.id} value={refImg.id}>
                                                                                                        {lib.name}: 图{refIdx + 1}{refImg.label ? ` (${refImg.label})` : ''}
                                                                                                    </option>
                                                                                                ))}
                                                                                            </select>
                                                                                        );
                                                    ...
                                                    {(() => {
                                                        ...
                                                        return (
                                                            <>
                                                                ...
                                                                {/* 合并调用开关 */}
                                                                <button
                                                                    onClick={() => setState(prev => ({ ...prev, batchAdvancedMode: !batchAdvancedMode }))}
                                                                    className={`${btnBase} tooltip-bottom ${batchAdvancedMode
                                                                        ? 'text-amber-300 bg-amber-900/30 border-amber-800/40'
                                                                        : 'text-zinc-400 bg-zinc-800/40 border-zinc-700/40'
                                                                        }`}
                                                                    data-tip={batchAdvancedMode ? "合并模式：所有随机组合合并为1次API调用（更快）" : "逐个模式：每个随机组合单独调用API（更准确）"}
                                                                >
                                                                    {batchAdvancedMode ? '⚡' : '🔗'}
                                                                    {batchAdvancedMode ? '合并' : '逐个'}
                                                                </button>
                                ...
                                {/* 多图拖拽选择弹窗 */}
                                {pendingDropFiles && pendingDropFiles.length >= 2 && (
                                    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setPendingDropFiles(null)}>
                                        <div className="bg-zinc-900 border border-zinc-700 rounded-xl shadow-2xl p-5 max-w-sm w-full mx-4" onClick={e => e.stopPropagation()}>
                                            <h3 className="text-white font-semibold text-sm mb-2">📷 检测到 {pendingDropFiles.length} 张图片</h3>
                                            <p className="text-zinc-400 text-xs mb-4">请选择如何处理这些图片：</p>
                                            <div className="flex flex-col gap-2">
                                                <button
                                                    onClick={() => {
                                                        const files = pendingDropFiles;
                                                        setPendingDropFiles(null);
                                                        handleFilesAsOneCard(files);
                                                    }}
                                                ...
                                                <button
                                                    onClick={() => {
                                                        const files = pendingDropFiles;
                                                        setPendingDropFiles(null);
                                                        handleFilesInternal(files);
                                                    }}
                                                    className="w-full py-2.5 px-4 rounded-lg bg-blue-600/20 border border-blue-500/40 text-blue-300 hover:bg-blue-600/30 transition-colors text-sm font-medium text-left"
                                                >
                                                    📄 每张图独立卡片
                                                    <span className="block text-[10px] text-blue-400/60 mt-0.5">{pendingDropFiles.length} 张图各自创建卡片</span>
                                                </button>
                                                <button
                ...
                {workMode === 'quick' && quickViewMode === 'standalone' ? (
                    <div className="flex-1 overflow-hidden">
                        <QuickModeStandalone
                            ...
                            onSyncLibraries={async () => {
                                const url = randomLibraryConfig.sourceSpreadsheetUrl;
                                if (!url) return;
                                const spreadsheetId = extractSpreadsheetId(url);
                                if (!spreadsheetId) return;

                                setQuickSyncingLibraries(true);
                                try {
                                    // 获取当前使用的总库分页名列表
                                    const currentSheets = new Set<string>();
                                    randomLibraryConfig.libraries.forEach(lib => {
                                        if (lib.sourceSheet) currentSheets.add(lib.sourceSheet);
                                    });
                                    const sheetsToRefresh = currentSheets.size > 0 ? Array.from(currentSheets) : [randomLibraryConfig.activeSourceSheet].filter(Boolean) as string[];

                                    const refreshedSheets = await scanMasterSheets(spreadsheetId, sheetsToRefresh);
                                    if (refreshedSheets.length === 0) {
                                        console.log('[快捷同步] 未找到数据');
                                        return;
                                    }
                                    ...
                                    // 更新配套指令
                                    const linkedInstructions: Record<string, string> = { ...randomLibraryConfig.linkedInstructions };
                                    for (const masterSheet of refreshedSheets) {
                                    ...
                                    console.log('[快捷同步] 完成，更新了', updatedLibraries.length, '个库');
                                } catch (error) {
                                    console.error('[快捷同步] 失败:', error);
                                } finally {
                                    setQuickSyncingLibraries(false);
                                }
                            }}
                            ...
                            onClearTextCards={() => { setTextCards([]); setIsGeneratingNoImage(false); }}
            ...
            {extractPromptModal && (
                <div
                    className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[220] flex items-center justify-center p-4"
                    onClick={() => setExtractPromptModal(null)}
                >
                    <div
                        className="w-full max-w-2xl bg-zinc-900 border border-cyan-700/40 rounded-2xl shadow-2xl overflow-hidden"
                        onClick={e => e.stopPropagation()}
                    >
                        <div className="px-5 py-4 border-b border-cyan-800/30 flex items-center justify-between bg-gradient-to-r from-cyan-900/20 to-transparent">
                            <div>
                                <h3 className="text-base font-bold text-cyan-300">提取指令弹框编辑</h3>
                                <p className="text-xs text-zinc-500 mt-0.5">
                                    {extractPromptModal.libName} · {extractPromptModal.mode === 'image' ? '参考图模式' : '逐图提取模式'}
                                </p>
                            </div>
                            <button onClick={() => setExtractPromptModal(null)} className="text-zinc-400 hover:text-white transition-colors">
                                <X size={18} />
                            </button>
                        </div>
                        <div className="p-5">
                            <textarea
                                value={quickOverrides[extractPromptModal.libName]?.extractPrompt || getDefaultExtractPrompt(extractPromptModal.libName)}
                                onChange={(e) => setQuickOverrides(prev => {
                                    const libName = extractPromptModal.libName;
                                    const mode = extractPromptModal.mode;
                                    const current = prev[libName] || { value: '', count: 0, mode };
                                    return {
                                        ...prev,
                                        [libName]: {
                                            ...current,
                                            value: mode === 'queue-image' ? '' : (current.value || ''),
                                ...
                                onKeyDown={(e) => {
                                    if (e.key === 'Escape') setExtractPromptModal(null);
                                }}
                                className="w-full min-h-[240px] px-3 py-2.5 text-sm rounded-lg bg-zinc-950 border border-zinc-700 text-zinc-200 outline-none focus:border-cyan-600 resize-y leading-relaxed"
                                autoFocus
                            />
                        </div>
                        <div className="px-5 py-3 border-t border-zinc-800 flex items-center justify-end">
                            <button
                                onClick={() => setExtractPromptModal(null)}
                                className="px-4 py-2 text-sm font-medium text-cyan-200 bg-cyan-700/30 border border-cyan-600/40 hover:bg-cyan-700/40 rounded-lg transition-colors"
                            >
                                完成
                            </button>
                        </div>
