...
你只需要给我最终的AI描述词就行，不需要其他任何多余的内容。并且英文回复我。` },
    ...
    {
        id: '6',
        name: '图片转为AI提示词-3（精准复刻，禁止复刻有版权的图）',
        text: `请作为视觉工程专家，对上传的图片进行“三层级逆向解构”，并为 Nano Banana 2 生成精确的复刻提示词：

第一层：底层架构 (Foundational Layer)

构图 (Framing): 景别（特写/中景/全景）、构图比例（9:16/16:9）、对称性、视角（平视/仰视）。
光影 (Lighting): 主光源方向、光质（柔光/硬光）、阴影深度、氛围色调（暖色/冷色/神圣感）。

第二层：精细特征 (Detail Layer)

主体描述 (Subject): 人物/物体的动作、表情、服装纹理（如缎面、金属、皮肤质感）。
环境背景 (Environment): 背景的深度、元素、焦外虚化程度（Bokeh）。

第三层：图文布局与样式 (Layout & UI Layer)

OCR 文字: 精确提取文字内容。
排版逻辑: 字体风格（无衬线/粗体）、文字位置、是否有遮罩/色块背景、品牌 Logo 坐标等等。

最后，请将以上分析合并为一段【Image-to-Image / Layout Reconstruction】提示词，要求逻辑清晰、词汇专业。

禁止复刻有版权的图像内容（包括受版权保护的角色、品牌素材、受保护作品构图与元素的高相似重建）；若检测到版权风险，请改为输出“风格借鉴但不构成复刻”的安全替代描述。
