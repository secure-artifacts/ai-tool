...
import {
    parseOpalWorkflow,
    convertToRandomLibraries,
    combineInstructions,
    OpalParseResult,
} from '../services/opalParserService';
...
export const RandomLibraryManager: React.FC<RandomLibraryManagerProps> = ({
    ...
    // Opal 画布文件导入相关状态
    const [showOpalImportModal, setShowOpalImportModal] = useState(false);
    const [opalParsing, setOpalParsing] = useState(false);
    const [opalError, setOpalError] = useState<string | null>(null);
    const [opalParseResult, setOpalParseResult] = useState<OpalParseResult | null>(null);
    const [opalSelectedLibs, setOpalSelectedLibs] = useState<Set<number>>(new Set());
    const [opalSelectedInstr, setOpalSelectedInstr] = useState<number>(0);
    const [opalImportMode, setOpalImportMode] = useState<'merge-add' | 'replace'>('replace');
    const opalFileInputRef = React.useRef<HTMLInputElement>(null);
        ...
        <div className="space-y-4 classic-mode-scroll-skin">
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button
                        onClick={() => {
                            setShowOpalImportModal(true);
                            setOpalParseResult(null);
                            setOpalSelectedLibs(new Set());
                            setOpalSelectedInstr(0);
                        }}
                        className="flex items-center gap-1 px-2 py-1 text-xs text-amber-400 hover:text-amber-300 bg-amber-900/20 hover:bg-amber-800/30 rounded border border-amber-800/30"
                        title="导入 Opal / AI Studio 画布工作流 JSON 文件，AI 自动解析指令和素材库"
                    >
                        <Sparkles size={12} />
                        导入画布
                    </button>
                    <input
                        ref={opalFileInputRef}
                        type="file"
                        accept="*"
                        className="hidden"
                        onChange={async (e) => {
                            ...
                            try {
                                ...
                                console.log('[OpalParser] 文件读取成功，长度:', text.length);
                                const result = await parseOpalWorkflow(text);
                                console.log('[OpalParser] 解析结果:', result);
                                setOpalParseResult(result);
                                setOpalSelectedLibs(new Set(result.libraries.map((_, i) => i)));
                                toast.success(`🎉 解析完成！发现 ${result.instructions.length} 条指令，${result.libraries.length} 个素材库`);
                            } catch (error: any) {
                                console.error('[OpalParser] 解析失败:', error);
                                const errMsg = error.message || '未知错误';
                                setOpalError(errMsg);
                                toast.error(`解析失败: ${errMsg}`);
                            } finally {
                                setOpalParsing(false);
                            }
                        }}
                    />
                    {/* 同步刷新按钮 */}
                ...
                <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={() => { if (!opalParsing) { setShowOpalImportModal(false); setOpalParseResult(null); setOpalSelectedLibs(new Set()); } }}>
                    <div className="bg-zinc-900 rounded-2xl border border-zinc-700 w-full max-w-3xl max-h-[85vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-4">
                            ...
                            {!opalParseResult && (
                                <div
                                    className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all ${opalParsing
                                        ? 'border-amber-500/50 bg-amber-900/10'
                                        : 'border-zinc-600 hover:border-amber-500/50 hover:bg-amber-900/10'
                                        }`}
                                    onClick={() => !opalParsing && opalFileInputRef.current?.click()}
                                    onDragOver={(e) => { e.preventDefault(); e.stopPropagation(); }}
                                    onDrop={async (e) => {
                                        e.preventDefault();
                                        e.stopPropagation();
                                ...
                                >
                                    {opalParsing ? (
                                        <div className="flex flex-col items-center gap-3">
                                            ...
                                            <p className="text-amber-300 font-medium">AI 正在分析工作流...</p>
                                            <p className="text-xs text-zinc-500">正在提取指令和素材库，请稍候</p>
                                        </div>
                                    ) : opalError ? (
                                        <div className="flex flex-col items-center gap-3">
                                            <div className="text-red-400 text-2xl">⚠️</div>
                                            <p className="text-red-300 font-medium">解析失败</p>
                                            <p className="text-xs text-red-400/80 max-w-md text-center">{opalError}</p>
                                            <p className="text-xs text-zinc-500 mt-2">点击重新选择文件</p>
                                        </div>
                                    ) : (
                                        <div className="flex flex-col items-center gap-3">
                                            <Upload size={32} className="text-zinc-500" />
                                            <p className="text-zinc-300">点击选择或拖放画布 JSON 文件到这里</p>
                                            <p className="text-xs text-zinc-500">支持 Opal / AI Studio 导出的 .json 工作流配置文件</p>
                                        </div>
                                    )}
                                </div>
                            )}
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="p-3 bg-amber-900/20 border border-amber-700/30 rounded-lg">
                                        <div className="flex items-center justify-between mb-1">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-sm font-medium text-amber-300">{opalParseResult.title}</span>
                                            </div>
                                            <div className="flex items-center gap-2">
                                                <button
                                                    onClick={() => {
                                                        let allText = `=== ${opalParseResult.title} ===\n${opalParseResult.summary}\n`;
                                                        if (opalParseResult.instructions.length > 0) {
                                                            allText += '\n\n========== 指令 ==========\n';
                                                            opalParseResult.instructions.forEach((instr) => {
                                                                allText += `\n--- [${instr.name}] ---\n${instr.text}\n`;
                                                            });
                                                        }
                                                        if (opalParseResult.libraries.length > 0) {
                                                            allText += '\n\n========== 素材库 ==========\n';
                                                            opalParseResult.libraries.forEach(lib => {
                                                                allText += `\n【${lib.name}】(${lib.items.length} 条)\n`;
                                                        ...
                                                        const libTotal = opalParseResult.libraries.reduce((s, l) => s + l.items.length, 0);
                                                        toast.success(`已复制全部结果（${opalParseResult.instructions.length} 条指令 + ${opalParseResult.libraries.length} 个库共 ${libTotal} 条）`);
                                                    }}
                                                    className="flex items-center gap-1 px-2 py-1 text-xs text-amber-300 hover:text-amber-200 bg-amber-800/30 hover:bg-amber-700/40 rounded border border-amber-700/40"
                                                >
                                                    <Copy size={12} />
                                                    复制全部
                                                </button>
                                                {opalParseResult.libraries.length > 0 && (
                                                    <button
                                                        onClick={() => {
                                                            // 构建 TSV 表格格式：列名=库名，行=条目
                                                            const libs = opalParseResult.libraries;
                                                            const maxRows = Math.max(...libs.map(l => l.items.length));
                                                            // 表头
                                                            let tsv = libs.map(l => l.name).join('\t') + '\n';
                                                            // 数据行
                                                            for (let row = 0; row < maxRows; row++) {
                                                                tsv += libs.map(l => l.items[row] || '').join('\t') + '\n';
                                                            }
                                                            navigator.clipboard.writeText(tsv);
                                                            const libTotal = libs.reduce((s, l) => s + l.items.length, 0);
                                                            toast.success(`已复制 ${libs.length} 个库（${libTotal} 条）为表格格式，可直接粘贴到 Sheets`);
                                                        }}
                                                        className="flex items-center gap-1 px-2 py-1 text-xs text-green-300 hover:text-green-200 bg-green-800/30 hover:bg-green-700/40 rounded border border-green-700/40"
                                                    >
                                                        <Copy size={12} />
                                                        复制为表格
                                                    </button>
                                        ...
                                        <p className="text-xs text-zinc-400">{opalParseResult.summary}</p>
                                    </div>

                                    {/* 指令列表 */}
                                    {opalParseResult.instructions.length > 0 && (
                                        <div className="space-y-2">
                                            <div className="flex items-center justify-between">
                                                <h4 className="text-sm font-medium text-white flex items-center gap-2">
                                                    📋 发现 {opalParseResult.instructions.length} 条指令
                                                </h4>
                                                <button
                                                    onClick={() => {
                                                        const allText = opalParseResult.instructions.map(i => `[指令: ${i.name}]\n${i.text}`).join('\n\n---\n\n');
                                                        navigator.clipboard.writeText(allText);
                                                        toast.success('已复制所有指令到剪贴板');
                                                    }}
                                            ...
                                            {opalParseResult.instructions.map((instr, idx) => (
                                                <div
                                                    key={idx}
                                                    className={`p-3 rounded-lg border cursor-pointer transition-all ${opalSelectedInstr === idx
                                                        ? 'border-purple-500 bg-purple-900/20'
                                                        : 'border-zinc-700 bg-zinc-800/50 hover:border-zinc-600'
                                                        }`}
                                                    onClick={() => setOpalSelectedInstr(idx)}
                                                >
                                                    <div className="flex items-center justify-between mb-1">
                                                        <span className="text-xs font-medium text-purple-300">{instr.name}</span>
                                                        <div className="flex items-center gap-2">
                                                            <span className="text-xs text-zinc-500">{instr.text.length} 字</span>
                                                            <button
                                                                onClick={(e) => {
                                                                    e.stopPropagation();
                                                                    navigator.clipboard.writeText(instr.text);
                                                                    toast.success(`已复制指令「${instr.name}」`);
                                                                }}
                                                                className="p-1 text-zinc-500 hover:text-white rounded hover:bg-zinc-600"
                                                                title="复制该指令"
                                                            >
                                                                <Copy size={12} />
                                                            </button>
                                                    ...
                                                    <p className="text-xs text-zinc-400 line-clamp-3 whitespace-pre-wrap">{instr.text.substring(0, 300)}{instr.text.length > 300 ? '...' : ''}</p>
                                                </div>
                                            ))}
                                        </div>
                                    )}
                                    ...
                                    {opalParseResult.libraries.length > 0 && (
                                        <div className="space-y-2">
                                            <div className="flex items-center justify-between">
                                                <h4 className="text-sm font-medium text-white flex items-center gap-2">
                                                    📦 发现 {opalParseResult.libraries.length} 个素材库
                                                    <span className="text-xs text-zinc-500">
                                                        (共 {opalParseResult.libraries.reduce((s, l) => s + l.items.length, 0)} 条)
                                                    </span>
                                                </h4>
                                                <div className="flex items-center gap-2">
                                                    <button
                                                        onClick={() => {
                                                            let text = '';
                                                            opalParseResult.libraries.forEach(lib => {
                                                                text += `【${lib.name}】(${lib.items.length} 条)\n`;
                                                                lib.items.forEach((item, i) => {
                                                                    text += `${i + 1}. ${item}\n`;
                                                                });
                                                                text += '\n';
                                                            ...
                                                            navigator.clipboard.writeText(text);
                                                            const total = opalParseResult.libraries.reduce((s, l) => s + l.items.length, 0);
                                                            toast.success(`已复制全部 ${opalParseResult.libraries.length} 个库（${total} 条）`);
                                                        }}
                                                        className="flex items-center gap-1 text-xs text-zinc-400 hover:text-white"
                                                    >
                                                        <Copy size={12} />
                                                    ...
                                                    <button
                                                        onClick={() => {
                                                            if (opalSelectedLibs.size === opalParseResult.libraries.length) {
                                                                setOpalSelectedLibs(new Set());
                                                            } else {
                                                                setOpalSelectedLibs(new Set(opalParseResult.libraries.map((_, i) => i)));
                                                            }
                                                        }}
                                                        className="text-xs text-zinc-400 hover:text-white"
                                                    >
                                                        {opalSelectedLibs.size === opalParseResult.libraries.length ? '取消全选' : '全选'}
                                                    </button>
                                                </div>
                                            </div>
                                            {opalParseResult.libraries.map((lib, idx) => (
                                                <label
                                                    key={idx}
                                                    className={`flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition-all ${opalSelectedLibs.has(idx)
                                                ...
                                                >
                                                    <input
                                                        type="checkbox"
                                                        checked={opalSelectedLibs.has(idx)}
                                                        onChange={() => {
                                                            const next = new Set(opalSelectedLibs);
                                                            if (next.has(idx)) next.delete(idx);
                                                            else next.add(idx);
                                                            setOpalSelectedLibs(next);
                                                        }}
                                                        className="w-4 h-4 mt-0.5 rounded border-zinc-600 bg-zinc-700 text-green-500 focus:ring-0"
                                                    />
                                                    <div className="flex-1 min-w-0">
                                                        <div className="flex items-center gap-2 mb-1">
                                                            <span className="text-sm font-medium text-white">{lib.name}</span>
                                                            <span className="text-xs px-1.5 py-0.5 rounded bg-zinc-700 text-zinc-300">{lib.items.length} 条</span>
                                                            <button
                                                                onClick={(e) => {
                                                                    e.preventDefault();
                                                                    e.stopPropagation();
                                                                    const text = lib.items.map((item, i) => `${i + 1}. ${item}`).join('\n');
                                                                    navigator.clipboard.writeText(text);
                                                                    toast.success(`已复制「${lib.name}」${lib.items.length} 条到剪贴板`);
                                                                }}
                                                                className="p-1 text-zinc-500 hover:text-white rounded hover:bg-zinc-600"
                                                                title={`复制${lib.name}全部内容`}
                                                            >
                                                                <Copy size={12} />
                                                            </button>
                                                        </div>
                                                        <p className="text-xs text-zinc-500 truncate">
                                                            预览: {lib.items.slice(0, 3).map(item => item.substring(0, 40)).join(' | ')}{lib.items.length > 3 ? ' ...' : ''}
                                                        </p>
                                                    </div>
                                                </label>
                                            ))}
                                    ...
                                    <div className="p-3 bg-zinc-800/60 rounded-lg border border-zinc-700/50">
                                        <label className="block text-xs text-zinc-400 mb-2">导入模式:</label>
                                        <div className="flex gap-4">
                                            <label className="flex items-center gap-2 text-xs text-zinc-300 cursor-pointer">
                                                <input
                                                    type="radio"
                                                    checked={opalImportMode === 'replace'}
                                                    onChange={() => setOpalImportMode('replace')}
                                            ...
                                            <label className="flex items-center gap-2 text-xs text-zinc-300 cursor-pointer">
                                                <input
                                                    type="radio"
                                                    checked={opalImportMode === 'merge-add'}
                                                    onChange={() => setOpalImportMode('merge-add')}
                                                    className="w-3.5 h-3.5 text-amber-500"
                                                />
                                                追加到现有库
                                            </label>
                        ...
                        {opalParseResult && (
                            <div className="flex items-center justify-between p-4 border-t border-zinc-700">
                                <button
                                    ...
                                    className="px-4 py-2 text-sm text-zinc-400 hover:text-white bg-zinc-800 hover:bg-zinc-700 rounded-lg"
                                >
                                    重新上传
                                </button>
                                <div className="flex items-center gap-3">
                                    <button
                                        onClick={() => setShowOpalImportModal(false)}
                                        className="px-4 py-2 text-sm text-zinc-400 hover:text-white"
                                    >
                                        取消
                                    </button>
                                    <button
                                        disabled={opalSelectedLibs.size === 0 && opalParseResult.instructions.length === 0}
                                        onClick={() => {
                                            // 导入选中的库
                                            const selectedLibraries = convertToRandomLibraries({
                                                ...opalParseResult,
                                                libraries: opalParseResult.libraries.filter((_, i) => opalSelectedLibs.has(i)),
                                            });

                                            let finalLibraries: RandomLibrary[];
                                            if (opalImportMode === 'replace') {
                                                finalLibraries = selectedLibraries;
                                            } else {
                                                const existingNames = new Set(config.libraries.map(l => l.name));
                                                const newLibs = selectedLibraries.filter(l => !existingNames.has(l.name));
                                                finalLibraries = [...config.libraries, ...newLibs];
                                            }

                                            // 构建 linkedInstructions
                                            const linkedInstructions: Record<string, string> = { ...config.linkedInstructions };
                                            const instrText = opalParseResult.instructions[opalSelectedInstr]?.text || combineInstructions(opalParseResult);
                                            const sourceLabel = `画布: ${opalParseResult.title}`;
                                            if (instrText) {
                                                linkedInstructions[sourceLabel] = instrText;
                                            }
                                            ...
                                            const libCount = selectedLibraries.length;
                                            const totalItems = selectedLibraries.reduce((s, l) => s + l.values.length, 0);
                                            toast.success(`✅ 成功导入 ${libCount} 个库 (共 ${totalItems} 条)${instrText ? '，已保存基础指令' : ''}！`);
                                            setShowOpalImportModal(false);
                                        }}
                                        className="px-6 py-2 text-sm font-medium bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-500 hover:to-orange-500 text-white rounded-lg disabled:opacity-40 disabled:cursor-not-allowed"
                                    >
                                        确认导入 ({opalSelectedLibs.size} 个库)
                                    </button>
                                </div>
                            </div>
                        )}
