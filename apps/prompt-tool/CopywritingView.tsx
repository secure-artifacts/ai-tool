...
If you are not ashamed to love Jesus, put Amen.`,
    ...
    },
    {
        ...
        name: '🎙️ 人声情感标签',
        instruction: `根据这个文案帮我加一些情感标签。要符合 ElevenLabs 这款软件生成音频使用。
            ...
            for (let attempt = 0; attempt <= 3; attempt++) {
                try {
                    ...
                    if (is429 && attempt < 3) {
                        ...
                        console.warn(`[文案工具] processItem 429限速，第${attempt + 1}次重试，等待${waitSec}s`);
...
按格式输出每条结果：[编号] 改写后的外文|||中文翻译`;
        ...
        try {
            ...
            for (let attempt = 0; attempt <= 3; attempt++) {
                try {
                    ...
                    if (is429 && attempt < 3) {
                        const waitSec = Math.pow(2, attempt + 1) * 2.5;
                        console.warn(`[文案工具] processBatch 429限速，第${attempt + 1}次重试，等待${waitSec}s`);
                        await new Promise(resolve => setTimeout(resolve, waitSec * 1000));
                        continue;
                    }
                    throw retryError;
                        ...
                        if (mode === 'classify') {
                            ...
                            if (!autoTranslate && !inst.includes('|||') && mode !== 'prayer' && mode !== 'voice') {
                                ...
                                if (parts.length >= 2) {
                                    results.set(item.id, {
                                        foreign: parts[0].trim(),
                                        chinese: mode === 'voice' ? parts[1].trim().replace(/\\n/g, '\n') : parts[1].trim(),
    ...
    const handleStartProcessing = async () => {
            ...
            const runNext = async () => {
                while (idx < freeformItems.length && !stopRef.current) {
                    ...
                    try {
                        ...
                        setItems(prev => prev.map(i =>
                            i.id === item.id ? { ...i, status: 'error' as const, error: error.message || '生成失败' } : i
                        ));
                    }
                }
            };
                    ...
                    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
                        try { return await fn(); } catch (error: any) {
                            ...
                            if (is429 && attempt < MAX_RETRIES) {
                                const waitSec = Math.pow(2, attempt + 1) * 2.5;
                                showCopyToast(`⏳ API 频率限制，等待 ${waitSec}s 后重试...`);
                                await new Promise(resolve => setTimeout(resolve, waitSec * 1000));
                                continue;
                            }
                            throw error;
        ...
        // 429 自动重试helper：遇到频率限制时自动等待后重试（所有模式共用）
        const MAX_RETRIES = 3;
        const callWithRetry = async (fn: () => Promise<any>, label: string): Promise<any> => {
            for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
                try {
                    ...
                    const is429 = msg.includes('429') || msg.includes('RESOURCE_EXHAUSTED') || msg.includes('quota') || msg.includes('rate limit');
                    if (is429 && attempt < MAX_RETRIES) {
                        const waitSec = Math.pow(2, attempt + 1) * 2.5; // 5s, 10s, 20s
                        console.warn(`[文案工具] ${label} 遇到429限速，第${attempt + 1}次重试，等待${waitSec}秒...`);
                        showCopyToast(`⏳ API 频率限制，等待 ${waitSec}s 后重试...`);
                        await new Promise(resolve => setTimeout(resolve, waitSec * 1000));
                        continue;
                    }
...
${allLibsPrompt}`;
                    ...
                    for (let attempt = 0; attempt <= 3; attempt++) {
                        try {
                            ...
                            if (is429 && attempt < 3) {
                                const waitSec = Math.pow(2, attempt + 1) * 2.5;
                                console.warn(`[文案工具] library模式 429限速，第${attempt + 1}次重试，等待${waitSec}s`);
                                await new Promise(resolve => setTimeout(resolve, waitSec * 1000));
                                continue;
                            }
                            throw retryError;
                            ...
                            for (let retryAttempt = 0; retryAttempt <= 3; retryAttempt++) {
                                try {
                                    ...
                                    if (is429 && retryAttempt < 3) {
                                        ...
                                        console.warn(`[文案工具] social-media 429限速，第${retryAttempt + 1}次重试，等待${waitSec}s`);
                                        await new Promise(resolve => setTimeout(resolve, waitSec * 1000));
                                        continue;
                                    }
                                    throw retryError;
...
请根据上述分类规则，只输出分类结果，不要附加任何解释或说明。`;
                            ...
                            for (let retryAttempt = 0; retryAttempt <= 3; retryAttempt++) {
                                try {
                                    ...
                                    if (is429 && retryAttempt < 3) {
                                        const waitSec = Math.pow(2, retryAttempt + 1) * 2.5;
                                        console.warn(`[文案工具] 标准模式 429限速，第${retryAttempt + 1}次重试，等待${waitSec}s`);
                                        await new Promise(resolve => setTimeout(resolve, waitSec * 1000));
                                        continue;
                                    }
                                    throw retryError;
    ...
    return (
        <div className="flex flex-col h-full bg-zinc-950 text-zinc-100 p-4 gap-3 overflow-y-auto overflow-x-hidden custom-scrollbar">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                            ...
                            <div className="flex flex-wrap items-center gap-0.5">
                                ...
                                <button
                                    ...
                                    className={`px-2 py-0.5 text-[10px] rounded-l-full transition-all border whitespace-nowrap ${mode === 'standard'
                                ...
                                <button
                                    ...
                                    className={`px-2 py-0.5 text-[10px] rounded-r-full transition-all border whitespace-nowrap ${mode === 'prayer'
                                ...
                                <button
                                    ...
                                    className={`px-2 py-0.5 text-[10px] rounded-r-full transition-all border whitespace-nowrap ${mode === 'split'
                        ...
                        <>
                            ...
                            <div className="mt-2 flex items-center gap-3 flex-wrap">
                                <button
                                    ...
                                    className="flex items-center gap-1 px-2 py-0.5 text-[10px] text-amber-400 hover:bg-amber-900/20 rounded border border-amber-900/30"
                                >
                                    <Plus size={10} /> 添加指令
                                </button>
                                {mode === 'standard' && (
                                    <div className="flex items-center gap-1.5">
                                        <span className="text-[10px] text-zinc-500">每指令结果数:</span>
                                        {[1, 2, 3, 5].map(n => (
                                            <button
                                                key={n}
                                                onClick={() => setRewriteVariantCount(n)}
                                                className={`px-1.5 py-0.5 text-[10px] rounded transition-all border ${
                                        ...
                                        <input
                                            ...
                                            className="w-10 bg-zinc-800 border border-zinc-700 rounded px-1 py-0.5 text-[10px] text-zinc-200 text-center focus:outline-none focus:border-purple-500"
                                        />
                                    </div>
                                )}
                            </div>
            ...
            <div className="flex flex-wrap items-start gap-2">
                {/* 状态栏 */}
                {items.length > 0 ? (
                    <div className="flex items-stretch gap-0 bg-zinc-900 border border-zinc-800 rounded-lg overflow-hidden shrink-0" style={{ minWidth: '200px', maxWidth: '320px' }}>
                        <div className="flex-1 px-3 py-1.5 border-r border-zinc-800 whitespace-nowrap">
                            <span className="text-zinc-500 text-[10px]">队列</span>
                            <span className="ml-1.5 text-zinc-200 font-bold text-xs">{stats.total}</span>
                        </div>
                        <div className="flex-1 px-3 py-1.5 border-r border-amber-900/30 bg-amber-900/10 whitespace-nowrap">
                            <span className="text-amber-400 text-[10px]">待处理</span>
                            <span className="ml-1.5 text-amber-400 font-bold text-xs">{stats.idle}</span>
                        </div>
                        <div className="flex-1 px-3 py-1.5 border-r border-emerald-900/30 bg-emerald-900/10 whitespace-nowrap">
                        ...
                        <div className="flex-1 px-3 py-1.5 bg-red-900/10 whitespace-nowrap">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {isProcessing ? (
                            ...
                            <button
                                ...
                                className={`flex items-center gap-1 px-3 py-1.5 ${mode === 'split' ? 'bg-orange-600 hover:bg-orange-500' : mode === 'library' ? 'bg-green-600 hover:bg-green-500' : mode === 'social-media' ? 'bg-teal-600 hover:bg-teal-500' : mode === 'parallel' ? 'bg-rose-600 hover:bg-rose-500' : mode === 'cleaner' ? 'bg-lime-600 hover:bg-lime-500' : mode === 'prayer' ? 'bg-sky-600 hover:bg-sky-500' : 'bg-purple-600 hover:bg-purple-500'} text-white rounded text-xs font-medium disabled:opacity-50`}
                            >
                            ...
                            {mode === 'split' ? (
                                ...
                                <>
                                    ...
                                    >
                                        ...
                                        {mode === "voice" ? '标签+断句' : mode === 'library' ? '结果+翻译' : mode === 'cleaner' ? '结果四列' : mode === 'social-media' ? socialMediaOutputSections.filter(s => s.enabled).map(s => s.name).join('+') : rewriteVariantCount > 1 ? `结果${rewriteVariantCount * 2}列` : '结果两列'}
                                    </button>
                                    <button
                                    ...
                                    >
                                        ...
                                        {mode === 'library' ? '全部列' : mode === 'cleaner' ? '全部五列' : '全部四列'}
                                    </button>
