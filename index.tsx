...
const translations = {
  en: {
    ...
    uploadPrompt: "2. Click or drag to upload, or paste an image",
...
type ImageToPromptToolProps = {
  state: PromptToolState;
  setState: React.Dispatch<React.SetStateAction<PromptToolState>>;
  templateBuilderState: TemplateBuilderState;
  setTemplateBuilderState: React.Dispatch<React.SetStateAction<TemplateBuilderState>>;
  descState: DescState;
...
const ImageToPromptTool: React.FC<ImageToPromptToolProps> = ({
  state,
  setState,
  templateBuilderState,
  setTemplateBuilderState,
  descState,
  setDescState,
  onNavigateToDesc,
  ...
  const { t } = useTranslation();
  const { getAiInstance } = useApi();
  const { sessions, activeSessionId, activeImageId, userInput, error } = state;
  const DEFAULT_MERGED_ENGLISH = 'No English prompts generated yet.';
  const DEFAULT_MERGED_CHINESE = '暂无中文提示词。';
  const [isProcessing, setIsProcessing] = useState(false); // For individual chat messages
  const [isBatchProcessing, setIsBatchProcessing] = useState(false); // For the main batch job
  ...
  const [selectedTemplateVersionId, setSelectedTemplateVersionId] = useState<string>(
    templateBuilderState.activeVersionId || templateBuilderState.savedTemplates[0]?.id || ''
  );

  // 批量处理模式：'accurate'（精确模式，逐张处理）或 'fast'（快速模式，批量打包）
  const [batchMode, setBatchMode] = useState<'accurate' | 'fast'>(() => {
    if (typeof window === 'undefined') return 'accurate';
    try {
      return (localStorage.getItem('imageToPrompt_batchMode') as 'accurate' | 'fast') || 'accurate';
    } catch {
      return 'accurate';
    }
  });
  ...
  useEffect(() => {
    if (typeof window === 'undefined') return;
    try {
      localStorage.setItem('imageToPrompt_batchMode', batchMode);
    } catch (e) {
      console.warn('Failed to save batch mode', e);
    }
  }, [batchMode]);

  const handleModeChange = (newMode: 'accurate' | 'fast') => {
    if (newMode === 'fast' && batchMode !== 'fast') {
      // 切换到快速模式时显示警告弹框
  ...
  const [manualInstruction, setManualInstruction] = useState<string>(() => {
    if (typeof window === 'undefined') return '';
    try {
      return localStorage.getItem('imageToPrompt_manualInstruction') || '';
    } catch {
      return '';
    }
  });
  ...
  useEffect(() => {
    if (typeof window === 'undefined') return;
    try {
      localStorage.setItem('imageToPrompt_manualInstruction', manualInstruction);
    } catch (e) {
  ...
  const [systemInstructionCopied, setSystemInstructionCopied] = useState(false);

  const handleViewSystemInstruction = () => {
    setShowSystemInstructionModal(true);
  };

  const handleCopySystemInstruction = () => {
    const systemInstruction = getMultiExpertSystemInstruction(selectedExperts);
    navigator.clipboard.writeText(systemInstruction).then(() => {
      setSystemInstructionCopied(true);
      setTimeout(() => setSystemInstructionCopied(false), 2000);
    });
  };
  ...
  const [selectedPromptTemplateId, setSelectedPromptTemplateId] = useState<string>(() => {
    ...
    try {
      return localStorage.getItem('imageToPrompt_selectedTemplateId') || 'system_default';
    } catch {
      return 'system_default';
    }
  });

  // 自定义反推指令相关状态（已废弃，现在使用模版系统）
  const [showInstructionEditor, setShowInstructionEditor] = useState(false);
  const [customInstruction, setCustomInstruction] = useState<string>(() => {
    if (typeof window === 'undefined') return '';
    try {
      return localStorage.getItem('imageToPrompt_customInstruction') || '';
    } catch {
      return '';
    }
  });
  ...
  // 保存反推模版选择到localStorage
  useEffect(() => {
    if (typeof window === 'undefined') return;
    try {
      localStorage.setItem('imageToPrompt_selectedTemplateId', selectedPromptTemplateId);
    } catch (e) {
      console.warn('Failed to save template selection', e);
    }
  }, [selectedPromptTemplateId]);

  const templateInstructionText = useMemo(
    () => buildCombinedTemplateText(templateBuilderState.sections, templateBuilderState.values),
    [templateBuilderState.sections, templateBuilderState.values]
  );

  // 获取当前选中的反推指令模版内容
  const getSelectedPromptTemplateText = () => {
    if (selectedPromptTemplateId === 'system_default') {
      return ''; // 空字符串表示使用系统默认
    }
    if (selectedPromptTemplateId === 'manual_input') {
      return manualInstruction.trim();
    }
    const selectedTemplate = templateBuilderState.savedTemplates.find(t => t.id === selectedPromptTemplateId);
    if (selectedTemplate) {
  ...
  const activeImage = activeSession?.images.find(img => img.id === activeImageId);
  const activeImageKey = activeImage?.id ?? null;
  ...
  const LOCAL_STORAGE_KEY = 'aetherius-prompt-tool-sessions';
  useEffect(() => {
    if (templateBuilderState.activeVersionId && templateBuilderState.activeVersionId !== selectedTemplateVersionId) {
      setSelectedTemplateVersionId(templateBuilderState.activeVersionId);
    }
  }, [templateBuilderState.activeVersionId, selectedTemplateVersionId]);

  const applyTemplateVersion = (versionId: string) => {
    setTemplateBuilderState(prev => {
      const version = prev.savedTemplates.find(template => template.id === versionId);
  ...
  useEffect(() => {
    try {
      const savedSessionsRaw = localStorage.getItem(LOCAL_STORAGE_KEY);
      if (savedSessionsRaw) {
        const savedSessions = JSON.parse(savedSessionsRaw);
        // Recreate blob URLs from base64 data
        const sessionsWithUrls = savedSessions.map((session: any) => ({
          ...session,
          images: session.images.map((img: any) => {
            const blob = b64toBlob(img.imageData.base64, img.imageData.type);
            return {
              ...img,
              imageData: {
                ...img.imageData,
        ...
        setState(prev => ({ ...prev, sessions: sessionsWithUrls }));
      }
    } catch (error) {
  ...
  // Save state to localStorage whenever sessions change
  useEffect(() => {
    try {
      // A simple check to avoid overwriting hydrated state with initial empty state
      if (sessions.length === 0 && localStorage.getItem(LOCAL_STORAGE_KEY) !== null) {
        const savedData = JSON.parse(localStorage.getItem(LOCAL_STORAGE_KEY)!);
        if (savedData.length > 0) return; // Don't overwrite if there's saved data
      }

      const sessionsToSave = sessions.map((session: any) => ({
        ...session,
        images: session.images.map((img: any) => {
          // Create a serializable version of imageData, excluding dynamic 'url' and bulky 'file' object
          const { url, file, ...serializableImageData } = img.imageData;
          return {
            ...img,
  ...
  }, [activeImage?.chatHistory]);

  const buildMergedPromptsFromSession = (session?: any) => {
    const defaultResult = {
      english: DEFAULT_MERGED_ENGLISH,
      chinese: DEFAULT_MERGED_CHINESE
    };
    if (!session) return defaultResult;

    const allEnglishPrompts: string[] = [];
    const allChinesePrompts: string[] = [];

    session.images.forEach((img: any) => {
      ...
      try {
        ...
        if (Array.isArray(prompts)) {
          prompts.forEach((p: any) => {
            if (p.englishPrompt) allEnglishPrompts.push(p.englishPrompt);
            if (p.chinesePrompt) allChinesePrompts.push(p.chinesePrompt);
          });
        }
      } catch (e) {
        // ignore parse errors
    ...
    return {
      english: allEnglishPrompts.length > 0 ? allEnglishPrompts.join('\n') : defaultResult.english,
      chinese: allChinesePrompts.length > 0 ? allChinesePrompts.join('\n') : defaultResult.chinese,
    };
  };

  const splitEnglishPrompts = (text: string) => {
    if (text === DEFAULT_MERGED_ENGLISH) return [];
    return text
      .split('\n')
      .map(line => line.trim())
      .filter(Boolean);
  ...
  const getPromptLinesFromSession = (session?: any) => splitEnglishPrompts(buildMergedPromptsFromSession(session).english);

  const mergedPrompts = useMemo(() => buildMergedPromptsFromSession(activeSession), [activeSession]);

  const latestModelPrompts = useMemo(() => {
    if (!activeImage) return [];
    const lastModelMessage = [...activeImage.chatHistory].reverse().find(msg => msg.sender === 'model');
    if (!lastModelMessage) return [];
    try {
      const prompts = JSON.parse(lastModelMessage.text);
      if (Array.isArray(prompts)) {
        return prompts
          .map((prompt: any) => {
            if (typeof prompt.englishPrompt === 'string' && prompt.englishPrompt.trim()) {
              return prompt.englishPrompt.trim();
            }
  ...
  const promptCandidates = useMemo(() => {
    if (latestModelPrompts.length) return latestModelPrompts;
    if (mergedPrompts.english === DEFAULT_MERGED_ENGLISH) return [];
    return mergedPrompts.english
      .split('\n')
      .map(line => line.trim())
      .filter(Boolean);
  }, [latestModelPrompts, mergedPrompts.english]);
  ...
  const getMergedPromptPayload = () => {
    const segments: string[] = [];
    if (mergedPrompts.english && mergedPrompts.english !== DEFAULT_MERGED_ENGLISH) {
      segments.push(mergedPrompts.english);
    }
    if (mergedPrompts.chinese && mergedPrompts.chinese !== DEFAULT_MERGED_CHINESE) {
      segments.push(mergedPrompts.chinese);
    }
    return segments.join('\n\n');
  };

  const handleCopyAllPrompts = () => {
    const payload = getMergedPromptPayload();
    copyTextToClipboard(payload);
  };

  const handleCopyActivePrompts = () => {
    const activeText = promptCandidates.length
      ? promptCandidates.join('\n')
      : mergedPrompts.english !== DEFAULT_MERGED_ENGLISH
        ? mergedPrompts.english
        : '';
    copyTextToClipboard(activeText);
  };

  const handleOpenDescTool = () => {
    onNavigateToDesc();
  };

  const handleSelectSession = (sessionId: string) => {
    const newActiveSession = sessions.find(s => s.id === sessionId);
    const firstImageId = newActiveSession?.images[0]?.id || null;
    setState(prev => ({ ...prev, activeSessionId: sessionId, activeImageId: firstImageId }));
  };

  const handleTemplateVersionChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    const versionId = event.target.value;
    setSelectedTemplateVersionId(versionId);
    applyTemplateVersion(versionId);
  };

  const handleNewSession = () => {
    setState(prev => ({ ...prev, activeSessionId: null, activeImageId: null }));
  };

  const handleDeleteSession = (sessionIdToDelete: string) => {
    setState(prevState => {
      const sessionToDelete = prevState.sessions.find(s => s.id === sessionIdToDelete);
      // Clean up blob URLs from the session being deleted
      sessionToDelete?.images.forEach(img => URL.revokeObjectURL(img.imageData.url));

      const newSessions = prevState.sessions.filter(s => s.id !== sessionIdToDelete);
      let newActiveId = prevState.activeSessionId;
      let newActiveImageId = prevState.activeImageId;
      if (prevState.activeSessionId === sessionIdToDelete) {
        newActiveId = newSessions.length > 0 ? newSessions[0].id : null;
        newActiveImageId = newSessions.length > 0 ? newSessions[0].images[0].id : null;
      }
      return { ...prevState, sessions: newSessions, activeSessionId: newActiveId, activeImageId: newActiveImageId };
    });
  };

  const createImageObjects = async (files: File[], sessionId: string) => {
  ...
  const handleBatchImageUpload = async (files: File[]) => {
    if (selectedExperts.length === 0) {
      setState(prev => ({ ...prev, error: t('error_selectExpert') }));
      return;
    }
    ...
    setState(prevState => ({
      ...
      activeImageId: sessionImages[0]?.id || null,
      userInput: {}
    }));
  };
  ...
  const processPastedText = async (text: string): Promise<File[]> => {
    if (!text) return [];
    ...
    // 正则匹配 =IMAGE("url") 格式 (支持双引号或单引号)
    const imageFormulaRegex = /=IMAGE\s*\(\s*["']([^"']+)["']\s*\)/i;

    lines.forEach(line => {
      // 1. 尝试匹配 Google Sheets IMAGE 公式
      const formulaMatch = line.match(imageFormulaRegex);
      if (formulaMatch && formulaMatch[1]) {
        potentialUrls.push(formulaMatch[1]);
        return;
      }

      // 2. 尝试匹配普通 URL (简单匹配 http/https 开头)
      const urlMatch = line.match(/https?:\/\/[^\s"']+/);
      if (urlMatch) {
        potentialUrls.push(urlMatch[0]);
      }
    });

    if (potentialUrls.length === 0) return [];

    const urlFiles = await Promise.all(potentialUrls.map(async (originalUrl) => {
      // 先解码 HTML 实体（如 &amp; -> &）
      const decodedUrl = decodeHtmlEntities(originalUrl);
      // First apply Gyazo, Imgur, and other image host conversions
      let url = processImageUrl(decodedUrl);

      // Then check if it's a Google Drive URL and convert it
      ...
      // 快速检查后缀或域名，判断是否可能是图片
      const isImageLike = /\.(jpeg|jpg|gif|png|webp|bmp|tiff|svg)$/i.test(url) ||
      ...
      try {
        ...
        // For octet-stream or missing mime, try to infer from URL extension
        let finalMime = mimeType;
        if (!finalMime || finalMime === 'application/octet-stream' || !finalMime.startsWith('image/')) {
          const extMatch = originalUrl.match(/\.(jpeg|jpg|gif|png|webp|bmp|tiff|svg)$/i);
          if (extMatch) {
        ...
        const filename = `pasted-image-${Date.now()}.${finalMime.split('/')[1] || 'png'}`;
        return new File([blob], filename, { type: finalMime });
      } catch (e) {
        console.warn('Failed to fetch pasted URL:', originalUrl, e);
        return null;
      }
    }));

    return urlFiles.filter(f => f !== null) as File[];
  };

  // Use a ref to track showUrlInputModal so the paste handler can access the latest value
  const showUrlInputModalRef = useRef(false);

  useEffect(() => {
    const handlePaste = async (event: ClipboardEvent) => {
      // Skip if pasting into an input or textarea (e.g., the URL modal's textarea)
      // But allow our hidden paste-capture textarea (aria-hidden="true")
      ...
      // Skip if the URL input modal is open
      if (showUrlInputModalRef.current) {
        return;
      }

      const items = event.clipboardData?.items;
      const imageFiles: File[] = [];
      let hasBinaryImages = false;
      ...
      if (text) {
        const urlFiles = await processPastedText(text);
        imageFiles.push(...urlFiles);
      }

      // 3. 处理 Google Sheets HTML 格式（包含 =IMAGE() 公式的单元格）
      const htmlData = event.clipboardData?.getData('text/html');
      if (htmlData && imageFiles.length === 0) {
        ...
        cellsWithFormula.forEach(cell => {
          ...
          if (formula) {
            ...
            const formulaMatch = formula.match(/=IMAGE\s*\(\s*["']([^"']+)["']\s*\)/i);
            if (formulaMatch && formulaMatch[1]) {
              extractedUrls.push(formulaMatch[1]);
            }
          }
        });
              ...
              if (valueObj && typeof valueObj === 'object') {
                const searchValue = (obj: any): void => {
                  if (typeof obj === 'string') {
                    ...
                    const formulaMatch = obj.match(/=IMAGE\s*\(\s*["']([^"']+)["']\s*\)/i);
                    if (formulaMatch && formulaMatch[1]) {
                      extractedUrls.push(formulaMatch[1]);
                    }
        ...
        if (extractedUrls.length > 0) {
          const urlFiles = await processPastedText(extractedUrls.join('\n'));
          imageFiles.push(...urlFiles);
        }
      }

      if (imageFiles.length > 0) {
        // 如果是从URL获取的图片，可能没有preventDefault，这里再次检查
        // 但由于是异步的，preventDefault可能已经失效（对于文本粘贴）。
        // 这通常没问题，因为我们主要关心的是把图片加进去。

        const currentSession = sessions.find(s => s.id === activeSessionId);
        if (!currentSession || currentSession.status !== 'staging') {
          handleBatchImageUpload(imageFiles);
        } else if (currentSession.status === 'staging') {
          handleAppendImages(imageFiles);
        }
      }
    };
  ...
  useEffect(() => {
    showUrlInputModalRef.current = showUrlInputModal;
  }, [showUrlInputModal]);

  const handleOpenUrlInput = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    setUrlInputValue('');
  ...
  const handleUrlInputSubmit = async () => {
    ...
    const files = await processPastedText(urlInputValue);
    if (files.length > 0) {
      const currentSession = sessions.find(s => s.id === activeSessionId);
      if (!currentSession || currentSession.status !== 'staging') {
        handleBatchImageUpload(files);
      } else if (currentSession.status === 'staging') {
        handleAppendImages(files);
      }
    } else {
      alert('未能提取到有效的图片链接');
    }
  ...
  const handleAppendImages = async (files: File[]) => {
    ...
    const newImages = await createImageObjects(files, activeSession.id);
    setState(prevState => ({
      ...prevState,
      sessions: prevState.sessions.map(s =>
        s.id !== prevState.activeSessionId
          ? s
  ...
  const handlePreModificationChange = (imageId: string, text: string) => {
    setState((prevState: any) => ({
      ...prevState,
      sessions: prevState.sessions.map(s =>
        s.id !== prevState.activeSessionId ? s : {
          ...s,
          images: s.images.map((img: any) =>
            img.id !== imageId ? img : { ...img, preModificationPrompt: text }
          )
        }
      )
    }));
  ...
  const saveCustomInstruction = () => {
    try {
      localStorage.setItem('imageToPrompt_customInstruction', customInstruction);
      alert('自定义指令已保存！');
    } catch (e) {
  ...
  const resetToDefaultInstruction = () => {
    if (confirm('确定要恢复为默认指令吗？')) {
      setCustomInstruction('');
      try {
        localStorage.removeItem('imageToPrompt_customInstruction');
        alert('已恢复为默认指令');
      } catch (e) {
  ...
  const getEffectiveInstruction = (expertKeys: ExpertKey[], isBatch: boolean) => {
    // 优先使用选中的指令模版
    const templateText = getSelectedPromptTemplateText();
    if (templateText.trim()) {
      return templateText;
    }
    // 否则使用系统默认指令
    return isBatch
      ? getBatchMultiExpertSystemInstruction(expertKeys)
      : getMultiExpertSystemInstruction(expertKeys);
  };

  const handleStartBatchProcessing = async (options?: { after?: (prompts: PromptInnovationPayload[]) => void }) => {
    if (!activeSession) return;
    setIsBatchProcessing(true);

    // Set session and all images to processing
    setState(prevState => ({
      ...prevState,
      sessions: prevState.sessions.map(s =>
        s.id !== prevState.activeSessionId ? s : {
          ...
          images: s.images.map(img => ({ ...img, status: 'processing' as ImageStatus }))
        }
      )
    }));

    const collectedEnglishPrompts: PromptInnovationPayload[] = [];

    try {
      const ai = getAiInstance();

      if (batchMode === 'accurate') {
        // 精确模式：逐张独立请求，100%不会串内容
        console.log(`🎯 [Image to Prompt - Accurate Mode] Processing ${activeSession.images.length} images one by one`);
        const systemInstruction = getEffectiveInstruction(activeSession.experts, false);

        for (let index = 0; index < activeSession.images.length; index++) {
          const img = activeSession.images[index];

          try {
            const imagePart = { inlineData: { mimeType: img.imageData.type, data: img.imageData.base64 } };
            const parts: ({ inlineData: { mimeType: string; data: string; }; } | { text: string; })[] = [imagePart];
            if (img.preModificationPrompt?.trim()) {
              parts.push({ text: img.preModificationPrompt.trim() });
            }

            console.log(`🖼️ [Image to Prompt - Accurate] Processing image ${index + 1}/${activeSession.images.length}`);
            const response = await ai.models.generateContent({
              model: textModel,
              contents: { parts },
              config: { systemInstruction, responseMimeType: "application/json", responseSchema: singleImageResponseSchema },
            });

            const prompts = JSON.parse(response.text);
            if (Array.isArray(prompts)) {
              prompts.forEach((p: any) => {
                if (typeof p.englishPrompt === 'string' && p.englishPrompt.trim()) {
                  collectedEnglishPrompts.push({ text: p.englishPrompt.trim(), imageId: img.id });
                }
              });
            }
            ...
            setState(prevState => ({
              ...
              sessions: prevState.sessions.map(s =>
                s.id !== prevState.activeSessionId ? s : {
                  ...s,
                  images: s.images.map(i => i.id === img.id ? { ...i, status: 'success' as const, chatHistory: [modelMessage] } : i)
                }
              )
            }));
          } catch (imgError: any) {
            console.error(`Error processing image ${index}:`, imgError);
            const errorMessage = imgError.message?.includes('API key is not set') ? t('error_apiKeyNotSet') : (imgError.message || t('error_failedToAnalyze'));
            setState(prevState => ({
              ...prevState,
              sessions: prevState.sessions.map(s =>
                s.id !== prevState.activeSessionId ? s : {
                  ...s,
                  images: s.images.map(i => i.id === img.id ? { ...i, status: 'error' as const, error: errorMessage } : i)
                }
              )
            }));
          }
        ...
        setState(prevState => ({
          ...prevState,
          sessions: prevState.sessions.map(s =>
            s.id !== prevState.activeSessionId ? s : { ...s, status: 'complete' as SessionStatus }
          )
        }));

      } else {
        // 快速模式：批量打包请求（带索引标记）
        console.log(`⚡ [Image to Prompt - Fast Mode] Processing ${activeSession.images.length} images in batch`);
        ...
        const parts = activeSession.images.flatMap((img, index) => {
          ...
          const imagePart = { inlineData: { mimeType: img.imageData.type, data: img.imageData.base64 } };
          if (img.preModificationPrompt?.trim()) {
            return [markerPart, imagePart, { text: img.preModificationPrompt.trim() }] as const;
          }
          return [markerPart, imagePart] as const;
        });

        const response = await ai.models.generateContent({
          model: textModel,
          contents: { parts: parts },
          config: { systemInstruction, responseMimeType: "application/json", responseSchema: batchImageResponseSchema }
        });
        ...
        const updatedImages = activeSession.images.map((img, index) => {
          ...
          if (imageResultPrompts && Array.isArray(imageResultPrompts)) {
            imageResultPrompts.forEach((p: any) => {
              if (typeof p.englishPrompt === 'string' && p.englishPrompt.trim()) {
                collectedEnglishPrompts.push({ text: p.englishPrompt.trim(), imageId: img.id });
              }
            });
            const modelMessage: Message = { sender: 'model', text: JSON.stringify(imageResultPrompts) };
            return { ...img, status: 'success' as const, chatHistory: [modelMessage] };
          } else {
            return { ...img, status: 'error' as const, error: 'AI did not return a result for this image.' };
          }
        });
        ...
        setState(prevState => ({
          ...prevState,
          sessions: prevState.sessions.map(s =>
            s.id !== prevState.activeSessionId ? s : { ...s, status: 'complete', images: updatedImages }
          )
        }));
      }

      options?.after?.(collectedEnglishPrompts);

    } catch (e: any) {
      console.error("Batch processing failed:", e);
      const errorMessage = e.message?.includes('API key is not set') ? t('error_apiKeyNotSet') : (e.message || t('error_failedToAnalyze'));
      setState(prevState => ({
        ...prevState,
        sessions: prevState.sessions.map(s =>
          s.id !== prevState.activeSessionId ? s : {
            ...s,
            status: 'complete',
            images: s.images.map(img => ({ ...img, status: 'error', error: errorMessage }))
          }
        )
      }));
    } finally {
  ...
  const retrySingleImageAPI = async (imageToRetry: any) => {
    ...
    console.log(`🖼️ [Image to Prompt - Retry] Using AI model: ${textModel}`);
    const response = await ai.models.generateContent({
      model: textModel,
      contents: { parts },
      config: { systemInstruction, responseMimeType: "application/json", responseSchema: singleImageResponseSchema },
    ...
    return { sender: 'model', text: response.text } as Message;
  };
  ...
  const handleRetryImage = async (imageId: string) => {
    ...
    // Set specific image to processing
    setState(prevState => ({
      ...prevState,
      sessions: prevState.sessions.map(s => s.id !== prevState.activeSessionId ? s : {
        ...s, images: s.images.map(img => img.id === imageId ? { ...img, status: 'processing', error: null } : img)
      })
    }));

    try {
      const modelMessage = await retrySingleImageAPI(imageToRetry);
      setState(prevState => ({
        ...prevState,
        sessions: prevState.sessions.map(s => s.id !== prevState.activeSessionId ? s : {
          ...s, images: s.images.map(img => img.id === imageId ? { ...img, status: 'success', chatHistory: [modelMessage] } : img)
        })
      }));
    } catch (e: any) {
      console.error(`Retrying image ${imageId} failed:`, e);
      const errorMessage = e.message.includes('API key is not set') ? t('error_apiKeyNotSet') : (e.message || t('error_retryFailed'));
      setState(prevState => ({
        ...prevState,
        sessions: prevState.sessions.map(s => s.id !== prevState.activeSessionId ? s : {
          ...s, images: s.images.map(img => img.id === imageId ? { ...img, status: 'error', error: errorMessage } : img)
        })
      }));
    }
  };
  ...
  const handleRetryAllFailed = async () => {
    if (!activeSession) return;
    const imagesToRetry = activeSession.images.filter(img => img.status === 'error');
    if (imagesToRetry.length === 0) return;

    setIsBatchProcessing(true); // Reuse existing batch processing loader

    // Set all failed images to 'processing'
    setState(prevState => ({
      ...prevState,
      sessions: prevState.sessions.map(s => s.id !== prevState.activeSessionId ? s : {
        ...s, images: s.images.map(img => img.status === 'error' ? { ...img, status: 'processing', error: null } : img)
      })
    }));

    for (const image of imagesToRetry) {
      try {
        const modelMessage = await retrySingleImageAPI(image);
        setState(prevState => ({
          ...prevState,
          sessions: prevState.sessions.map(s => s.id !== prevState.activeSessionId ? s : {
            ...s, images: s.images.map(img => img.id === image.id ? { ...img, status: 'success', chatHistory: [modelMessage] } : img)
          })
        }));
      } catch (e: any) {
        console.error(`Retrying image ${image.id} failed:`, e);
        const errorMessage = e.message.includes('API key is not set') ? t('error_apiKeyNotSet') : (e.message || t('error_retryFailed'));
        setState(prevState => ({
          ...prevState,
          sessions: prevState.sessions.map(s => s.id !== prevState.activeSessionId ? s : {
            ...s, images: s.images.map(img => img.id === image.id ? { ...img, status: 'error', error: errorMessage } : img)
          })
        }));
      }
    }
  ...
  const handleSendMessage = async () => {
    if (!activeImageId || activeImageId === 'merged' || !activeSession || !userInput[activeImageId]?.trim()) return;
    if (!activeImage) return;

    const localUserInput = userInput[activeImageId];
    const userMessage: Message = { sender: 'user', text: localUserInput };
    const finalHistoryForAPI = [...activeImage.chatHistory, userMessage];

    setState(prevState => {
      const newUserInput = { ...prevState.userInput };
      delete newUserInput[prevState.activeImageId!];
      return {
        ...prevState,
        sessions: prevState.sessions.map(s =>
          s.id !== prevState.activeSessionId ? s : { ...s, images: s.images.map(img => img.id !== prevState.activeImageId ? img : { ...img, chatHistory: finalHistoryForAPI }) }
        ),
    ...
    try {
      const ai = getAiInstance();
      const systemInstruction = getEffectiveInstruction(activeSession.experts, false);
      const imagePart = { inlineData: { mimeType: activeImage.imageData.type, data: activeImage.imageData.base64 } };

      const apiHistory = [
        { role: 'user', parts: [imagePart] },
        ...finalHistoryForAPI.map(msg => ({
          role: msg.sender === 'user' ? 'user' : 'model',
          parts: [{ text: msg.text }]
        })),
      ];

      console.log(`💬 [Image to Prompt - Chat] Using AI model: ${textModel}`);
      const response = await ai.models.generateContent({
        model: textModel,
        contents: apiHistory,
        config: { systemInstruction, responseMimeType: "application/json", responseSchema: singleImageResponseSchema },
      ...
      setState(prevState => ({
        ...prevState,
        sessions: prevState.sessions.map(s =>
          s.id !== prevState.activeSessionId ? s : { ...s, images: s.images.map(img => img.id !== prevState.activeImageId ? img : { ...img, chatHistory: [...finalHistoryForAPI, modelMessage] }) }
        )
      }));

    } catch (e: any) {
      const errorMessage = e.message.includes('API key is not set') ? t('error_apiKeyNotSet') : (e.message || t('error_sendMessage'));
  ...
  const handleExpertChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const expert = e.target.value as ExpertKey;
    setSelectedExperts(prev =>
      prev.includes(expert)
        ? prev.filter(ex => ex !== expert)
        : [...prev, expert]
    );
  };

  const handleExport = async () => {
    if (!activeSession || isExporting) return;
    setIsExporting(true);

    try {
      const workbook = new ExcelJS.Workbook();
      const worksheet = workbook.addWorksheet('Image Prompts');

      await populateWorksheet(worksheet, activeSession, workbook);

      const buffer = await workbook.xlsx.writeBuffer();
      const blob = new Blob([buffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      link.download = `prompts-session-${activeSession.id}.xlsx`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      ...
      setState(prev => ({ ...prev, error: t('error_exportExcel') }));
    } finally {
  ...
  const handleExportAll = async () => {
    ...
    try {
      const workbook = new ExcelJS.Workbook();
      const worksheet = workbook.addWorksheet('All Prompts');
      const sortedSessions = [...sessions].sort((a: any, b: any) => parseInt(a.id) - parseInt(b.id));

      const allExperts = [...new Set(sortedSessions.flatMap((s: any) => s.experts))] as ExpertKey[];

      const expertHeaders = allExperts.flatMap((expert: string) => [
        { header: `${expert} (English)`, key: `${expert}_en`, width: 50, style: { alignment: { wrapText: true, vertical: 'top' } } },
        { header: `${expert} (Chinese)`, key: `${expert}_cn`, width: 50, style: { alignment: { wrapText: true, vertical: 'top' } } },
      ]);
      ...
      for (const session of sortedSessions) {
        for (const image of session.images) {
          const sessionTime = new Date(parseInt(session.id)).toLocaleString();
          const rowData: any = { sessionTime, filename: image.imageData.name };

          const lastModelMessage = [...image.chatHistory].reverse().find(msg => msg.sender === 'model');
          if (lastModelMessage) {
            try {
              const prompts = JSON.parse(lastModelMessage.text);
              if (Array.isArray(prompts)) {
                prompts.forEach((p: any) => {
                  if (allExperts.includes(p.expert)) {
                    rowData[`${p.expert}_en`] = p.englishPrompt;
                    rowData[`${p.expert}_cn`] = p.chinesePrompt;
                  }
                });
              }
            } catch (e) { console.error("Could not parse prompts for excel export", e); }
          }

          const newRow = worksheet.addRow(rowData);
          newRow.height = 80;

          const imageExt = image.imageData.type.split('/')[1] || 'png';
          const imageId = workbook.addImage({
            base64: image.imageData.base64,
            extension: imageExt as 'jpeg' | 'png' | 'gif',
          });

          worksheet.addImage(imageId, {
            tl: { col: 1.1, row: newRow.number - 1 + 0.1 },
      ...
      link.download = `prompts-export-all-${dateString}.xlsx`;
      ...
      console.error("Export all sessions failed:", e);
      setState(prev => ({ ...prev, error: t('error_exportAll') }));
    } finally {
      setIsExporting(false);
    }
  };

  const populateWorksheet = async (worksheet: any, session: any, workbook: any) => {
    const expertHeaders = session.experts.flatMap((expert: string) => [
      { header: `${expert} (English)`, key: `${expert}_en`, width: 50 },
      { header: `${expert} (Chinese)`, key: `${expert}_cn`, width: 50 },
    ]);
    ...
    for (const [index, image] of session.images.entries()) {
      ...
      const rowData: any = { filename: image.imageData.name };
  ...
  const isLoading = isProcessing || isBatchProcessing;
  const descOverlayMessage = descState.controlNotice || (descState.isPaused ? t('descPauseNotice') : t('descStatusProcessing'));
  const handleDescPauseToggle = () => {
    descControlRef.current?.togglePause();
  };
  const handleDescStop = () => {
    descControlRef.current?.stop();
  };


  const renderContent = () => {
    const mergedPayload = getMergedPromptPayload();
    if (!activeSession) {
      return (
        <div className="input-area create-session-view">
          <div className="expert-selector-multi">
            <label>{t('selectExperts')}</label>
            <div className="expert-options">
              {Object.keys(expertDescriptions).map((key: ExpertKey) => (
                <div key={key} className="expert-option">
                  <input
                    type="checkbox"
                    id={`expert-${key}`}
                    value={key}
                    checked={selectedExperts.includes(key)}
                    onChange={handleExpertChange}
                  />
                  <label htmlFor={`expert-${key}`}>{key}</label>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="expert-selector-multi" style={{ marginTop: '1rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
              <label style={{ marginBottom: 0 }}>
                反推指令模版 <span style={{ fontSize: '0.85em', color: 'var(--text-muted-color)', fontWeight: 'normal' }}>(可选自定义)</span>
              </label>
              {selectedPromptTemplateId === 'system_default' && (
                <button
                  className="secondary-btn small"
                  onClick={handleViewSystemInstruction}
            ...
            >
              ...
              <option value="manual_input">手动输入 (自定义)</option>
              {templateBuilderState.savedTemplates.map(template => (
                <option key={template.id} value={template.id}>
                  {template.name || '未命名模版'}
                </option>
              ))}
            </select>
            {selectedPromptTemplateId === 'manual_input' && (
              <textarea
                value={manualInstruction}
                onChange={(e) => setManualInstruction(e.target.value)}
                placeholder="在此输入您的自定义反推指令..."
                style={{
                  width: '100%',
                  marginTop: '0.5rem',
                  padding: '0.5rem',
            ...
            <div style={{ fontSize: '0.85rem', color: 'var(--text-muted-color)', marginTop: '0.5rem' }}>
              {selectedPromptTemplateId === 'system_default'
                ? '使用系统内置的反推指令'
                : `使用 "${templateBuilderState.savedTemplates.find(t => t.id === selectedPromptTemplateId)?.name || '自定义'}" 模版`}
            </div>
          </div>

          {showSystemInstructionModal && (
            <div className="modal-overlay" onMouseDown={() => setShowSystemInstructionModal(false)}>
              <div className="modal-content" style={{ maxWidth: '800px', width: '90%' }} onMouseDown={e => e.stopPropagation()}>
                <div className="modal-header">
                  <h3>系统默认反推指令</h3>
                  <button className="modal-close-btn" onClick={() => setShowSystemInstructionModal(false)}>&times;</button>
                </div>
                <div className="modal-body" style={{ maxHeight: '60vh', overflowY: 'auto', padding: '1rem', backgroundColor: 'var(--bg-color)', borderRadius: '4px' }}>
                  <div style={{
                    marginBottom: '1rem',
                    padding: '0.75rem',
                    ...
                    <strong>💡 提示：</strong> 这个指令相当于直接在 Gemini 对话框发送信息的要求指令。如果反推的词不符合你的要求，你可以：
                    <ul style={{ margin: '0.5rem 0 0 1.2rem', padding: 0 }}>
                      <li>修改此指令（点击下方“复制并编辑”）；</li>
                      <li>在指令模版中添加自己的要求；</li>
                      <li>将此指令复制给 GPT 或 Gemini，让它们帮你优化修改。</li>
                    </ul>
                  </div>
                  <pre style={{ whiteSpace: 'pre-wrap', fontFamily: 'monospace', margin: 0 }}>
                    {getMultiExpertSystemInstruction(selectedExperts)}
                  </pre>
                </div>
                <div className="modal-footer" style={{ marginTop: '1rem', display: 'flex', justifyContent: 'flex-end', gap: '1rem' }}>
                  <button className="secondary-btn" onClick={() => setShowSystemInstructionModal(false)}>关闭</button>
                  <button className="primary" onClick={handleCopySystemInstruction}>
                    {systemInstructionCopied ? '已复制!' : '复制到剪贴板'}
                  </button>
                </div>
              </div>
            </div>
          ...
          <FileUploader onFileSelect={(files) => handleBatchImageUpload(files as File[])} multiple>
            <div className="uploader-content">
              <span>🖼️</span>
              <p>{t('uploadPrompt')}</p>
              <div className="upload-buttons">
                <button type="button" className="secondary-btn" onClick={(e) => {
                  // We stop propagation to prevent the label's onClick from firing the file input again.
                  e.stopPropagation();
                  (e.currentTarget.closest('.file-upload-label')?.querySelector('input[type="file"]') as HTMLInputElement)?.click();
                }}>{t('uploadFromComputer')}</button>
                <button type="button" className="secondary-btn" onClick={(e) => { e.stopPropagation(); handleOpenUrlInput(e); }}>从链接添加</button>
              </div>
            </div>
          </FileUploader>
      ...
      return (
        <div className="staging-layout">
          <div className="staging-main">
            <h3>{t('stagingTitle', { count: activeSession.images.length })}</h3>
            <div className="batch-image-grid staging-grid">
              {activeSession.images.map((img: any) => (
                <div key={img.id} className="batch-image-item-staging">
                  ...
                  <textarea
                    ...
                    placeholder={t('preModificationPlaceholder')}
                    value={img.preModificationPrompt || ''}
                    onChange={(e) => handlePreModificationChange(img.id, e.target.value)}
                    rows={2}
                  />
                </div>
              ))}
          ...
          <aside className="staging-sidebar">
            {/* 指令模版选择器 (Staging View) */}
            <div className="expert-selector-multi" style={{ marginBottom: '1rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                <label style={{ marginBottom: 0 }}>
                  反推指令模版 <span style={{ fontSize: '0.85em', color: 'var(--text-muted-color)', fontWeight: 'normal' }}>(可选自定义)</span>
                </label>
                {selectedPromptTemplateId === 'system_default' ? (
                  ...
                  <button
                    className="secondary-btn small"
                    onClick={() => {
                      setTemplateBuilderState(prev => ({ ...prev, activeVersionId: selectedPromptTemplateId }));
                      onNavigateToTemplate();
                    }}
                    style={{ padding: '2px 8px', fontSize: '0.8rem' }}
                  >
            ...
            <FileUploader onFileSelect={(files) => handleAppendImages(files as File[])} multiple>
              <div className="uploader-content">
                <span>➕</span>
                <p>{t('appendImages')}</p>
                <div className="upload-buttons">
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <span style={{ fontSize: '0.85rem', color: 'var(--text-muted-color)' }}>处理模式：</span>
                <button
                  type="button"
                  onClick={() => handleModeChange('accurate')}
                  title="🎯 精确模式：逐张独立请求，100%不会串内容，但速度较慢，API消耗略多"
                  style={{
                    padding: '4px 10px',
                    fontSize: '0.8rem',
                    borderRadius: '4px',
                    border: batchMode === 'accurate' ? '2px solid var(--primary-color)' : '1px solid var(--border-color)',
                    backgroundColor: batchMode === 'accurate' ? 'var(--primary-color)' : 'var(--control-bg-color)',
                    color: batchMode === 'accurate' ? 'white' : 'var(--text-color)',
                    cursor: 'pointer',
                    fontWeight: batchMode === 'accurate' ? 'bold' : 'normal',
                    transition: 'all 0.2s'
                  }}
                >
                  🎯 精确
                ...
                <button
                  ...
                  title="⚡ 快速模式：批量打包处理，速度快，API消耗少，但相似图片可能会串内容"
                  style={{
                    padding: '4px 10px',
                    fontSize: '0.8rem',
                    borderRadius: '4px',
                    border: batchMode === 'fast' ? '2px solid #f59e0b' : '1px solid var(--border-color)',
                    backgroundColor: batchMode === 'fast' ? '#f59e0b' : 'var(--control-bg-color)',
                    color: batchMode === 'fast' ? 'white' : 'var(--text-color)',
                    cursor: 'pointer',
                    fontWeight: batchMode === 'fast' ? 'bold' : 'normal',
                    transition: 'all 0.2s'
                  }}
                >
                  ⚡ 快速
              ...
              <div style={{
                fontSize: '0.75rem',
                color: batchMode === 'accurate' ? '#f59e0b' : 'var(--success-color)',
                display: 'flex',
                alignItems: 'center',
                gap: '0.25rem'
              }}>
                {batchMode === 'accurate' ? (
                  <>⚠️ 精确模式将发送 {activeSession?.images.length || 0} 次请求（每张图片1次），API额度消耗较多</>
                ) : (
                  <>✅ 快速模式仅发送 1 次请求，API额度消耗较少</>
                )}
              </div>
            </div>
            <button className="primary process-all-btn" onClick={handleStartBatchProcessing} disabled={isBatchProcessing}>
              {isBatchProcessing ? t('processing') : t('startPrompting')}
            </button>
            {error && <p className="error-message">{error}</p>}
          </aside>
        </div>
    ...
    // Status is 'processing' or 'complete'
    const hasFailedImages = activeSession?.images.some(img => img.status === 'error');
    ...
    return (
      <>
        ...
        <div className="global-copy-actions">
          <button className="secondary-btn" onClick={handleCopyAllPrompts} disabled={!mergedPayload}>
            {t('copyAllPrompts')}
          </button>
        </div>
          ...
          <div className="batch-prompt-tabs">
            {activeSession.images.map(img => (
              <button
                ...
                className={`tab-btn ${img.id === activeImageId ? 'active' : ''}`}
              >
                ...
                {(img.status === 'success' || img.status === 'error') && (
                  <button
                    ...
                    disabled={isBatchProcessing || img.status === 'processing'}
                    title={t('reGenerate')}
                  >
                    🔄
                  </button>
            ...
            <button
              onClick={() => setState(prev => ({ ...prev, activeImageId: 'merged' }))}
        ...
        {activeImageId === 'merged' ? (
          <div className="merged-prompt-view">
            <PromptDisplay title={t('allEnglishPrompts')} text={mergedPrompts.english} />
            <PromptDisplay title={t('allChinesePrompts')} text={mergedPrompts.chinese} />
          </div>
        ) : activeImage ? (
            ...
            <div className="output-area">
              ...
              <div className="chat-input">
                <input
                  ...
                  onChange={(e) => setState(prev => ({ ...prev, userInput: { ...prev.userInput, [activeImageId!]: e.target.value } }))}
                  placeholder={t('chatPlaceholder')}
                  onKeyPress={(e) => e.key === 'Enter' && !isLoading && handleSendMessage()}
                  disabled={isLoading || activeImage.status !== 'success'}
                />
                <button className="primary" onClick={handleSendMessage} disabled={isLoading || activeImage.status !== 'success' || !(userInput[activeImageId] || '').trim()}>{t('sendMessage')}</button>
              </div>
            </div>
          </div>
        ) : <p>Select an image to view its prompts.</p>}
      </>
    );
  }
  ...
  return (
    ...
    >
      ...
      <aside className="history-panel">
        <button className="new-session-btn" onClick={handleNewSession}>{t('newSession')}</button>
          ...
          {sessions.map(session => (
            ...
            >
              {session.images && session.images.length > 0 &&
                <img src={session.images[0].imageData.url} alt="History thumbnail" />
              }
              <div className="history-item-info">
                <span>{new Date(parseInt(session.id)).toLocaleString()}</span>
                <span className="history-item-expert">{session.experts.join(', ')}</span>
              </div>
              <button className="delete-session-btn" onClick={(e) => { e.stopPropagation(); handleDeleteSession(session.id); }}>&times;</button>
            </div>
          ))}
        </div>
      </aside>
      <div className="main-content">
        <ToolHeader
          ...
          actions={
            <>
              {activeSession?.images.some(img => img.status === 'error') && (
                <button
                  onClick={handleRetryAllFailed}
                  className="secondary-btn error-btn"
                  disabled={isBatchProcessing}
                >
                  {isBatchProcessing ? t('processing') : t('retryAllFailed')}
                </button>
              )}
              <button
      ...
      {showUrlInputModal && (
        <div className="modal-overlay" onClick={() => setShowUrlInputModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ width: '500px' }}>
            <h3>从链接添加图片</h3>
            <p style={{ marginBottom: '1rem', color: 'var(--text-muted-color)' }}>
              请输入图片链接或 Google Sheets <code>=IMAGE(...)</code> 公式，每行一个。
            </p>
            <textarea
              value={urlInputValue}
            ...
            <div className="modal-footer" style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem' }}>
              <button className="secondary-btn" onClick={() => setShowUrlInputModal(false)}>取消</button>
              <button className="primary" onClick={handleUrlInputSubmit} disabled={!urlInputValue.trim()}>
                添加图片
              </button>
            </div>
          </div>
      ...
      {/* 快速模式警告弹框 */}
      {showFastModeWarning && (
        <div className="modal-overlay" onClick={() => setShowFastModeWarning(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ width: '450px' }}>
            ...
            <div style={{ margin: '1rem 0', lineHeight: 1.6 }}>
              <p style={{ marginBottom: '0.75rem' }}>
                <strong>快速模式</strong>会将所有图片打包成一次请求发送给AI，这样做有以下优缺点：
              </p>
              <div style={{ marginBottom: '0.75rem', paddingLeft: '1rem' }}>
                <p style={{ color: 'var(--success-color)' }}>✅ 速度更快</p>
                <p style={{ color: 'var(--success-color)' }}>✅ API消耗更少</p>
                <p style={{ color: '#f59e0b' }}>⚠️ 相似图片可能会内容串联</p>
                <p style={{ color: '#f59e0b' }}>⚠️ 图片数量多时风险增加</p>
              </div>
              <p style={{ color: 'var(--text-muted-color)', fontSize: '0.9rem' }}>
                如果您处理的图片差异较大，或对准确性要求不高，可以使用快速模式。<br />
                如果图片内容相似或需要100%准确，建议使用精确模式。
              </p>
            </div>
            <div className="modal-footer" style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem' }}>
              <button className="secondary-btn" onClick={() => setShowFastModeWarning(false)}>
                取消，保持精确模式
              </button>
              <button
                className="primary"
...
const SubEmailGenerator: React.FC = () => {
  const { t } = useTranslation();
    ...
    switch (activeTool) {
      case 'prompt':
        return (
          <ImageToPromptTool
            state={promptState}
            setState={updatePromptState}
            templateBuilderState={templateBuilderState}
            setTemplateBuilderState={setTemplateBuilderState}
            descState={descState}
            setDescState={setDescState}
            onNavigateToDesc={() => setActiveTool('desc')}
            onNavigateToTemplate={() => setActiveTool('template')}
            descControlRef={descControlRef}
            textModel={textModel}
          />
        );
      case 'translate':
        ...
        <div className="header-toggle-container">
          ...
          >
            ...
            <span className="update-text">
              {language === 'zh' ? '更新 (01/17)' : 'Updates (01/17)'}
            </span>
          </button>
        </div>
      )}
